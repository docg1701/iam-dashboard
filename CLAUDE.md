...
7. **CRITICAL: Use Test Scripts** - Use dedicated test scripts:
...
8. **CRITICAL: Command Output Management**:
...
10. **Never invent data when commands fail or get truncated** - Use simpler commands with native flags
11. **Never use complex command chains** - Keep one tool per command for reliability
12. **Never use `eval` in scripts** - Command injection vulnerability, use direct execution
13. **Never skip input validation in production scripts** - All parameters must be validated
14. **Never ignore security vulnerabilities** - All Critical/High severity issues must be fixed
