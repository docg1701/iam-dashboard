...
class TestOCRProcessorTool:
    ...
    def ocr_tool(self) -> OCRProcessorTool:
        ...
        return OCRProcessorTool(min_confidence=60.0, dpi=200, preprocessing=True)
    ...
    def test_validate_dependencies_success(
        self, mock_tesseract_version: Mock, ocr_tool: OCRProcessorTool
    ) -> None:
    ...
    def test_preprocess_image_failure(
        self, ocr_tool: OCRProcessorTool, sample_image: np.ndarray
    ) -> None:
    ...
    @patch("fitz.open")
    def test_extract_page_image_success(
        self, mock_fitz_open: Mock, ocr_tool: OCRProcessorTool, sample_pdf_path: str
    ) -> None:
    ...
    @patch("fitz.open")
    def test_extract_page_image_failure(
        self, mock_fitz_open: Mock, ocr_tool: OCRProcessorTool, sample_pdf_path: str
    ) -> None:
    ...
    def test_perform_ocr_on_image_success(
        self,
        ...
        with (
            patch.object(ocr_tool, "preprocess_image", return_value=sample_image),
    ...
    def test_perform_ocr_on_image_failure(
        self,
        mock_image_to_string: Mock,
        ocr_tool: OCRProcessorTool,
        sample_image: np.ndarray,
    ...
    @patch("fitz.open")
    def test_process_pdf_pages_success(
        self, mock_fitz_open: Mock, ocr_tool: OCRProcessorTool, sample_pdf_path: str
    ) -> None:
        ...
        # Mock methods
        with (
            patch.object(ocr_tool, "extract_page_image") as mock_extract,
    ...
    @patch("fitz.open")
    def test_process_pdf_pages_partial_success(
        self, mock_fitz_open: Mock, ocr_tool: OCRProcessorTool, sample_pdf_path: str
    ) -> None:
        ...
        with (
            ...
            def side_effect_extract(doc, page_num):
                if page_num == 0:  # First page succeeds
                    ...
                    return {"success": False, "error": "Image extraction failed"}
    ...
    def test_process_pdf_pages_invalid_pages(
        self, ocr_tool: OCRProcessorTool, sample_pdf_path: str
    ) -> None:
    ...
    def test_process_pdf_pages_failure(
        self, mock_fitz_open: Mock, ocr_tool: OCRProcessorTool, sample_pdf_path: str
    ) -> None:
    ...
    def test_process_document_with_text_analysis_no_ocr_needed(
        self, ocr_tool: OCRProcessorTool, sample_pdf_path: str
    ) -> None:
        ...
        result = ocr_tool.process_document_with_text_analysis(
            sample_pdf_path, text_content
        )
    ...
    def test_process_document_with_text_analysis_ocr_needed(
        self, ocr_tool: OCRProcessorTool, sample_pdf_path: str
    ) -> None:
        ...
        with patch.object(ocr_tool, "process_pdf_pages") as mock_process:
            ...
            result = ocr_tool.process_document_with_text_analysis(
                sample_pdf_path, text_content, char_threshold=100
            )
