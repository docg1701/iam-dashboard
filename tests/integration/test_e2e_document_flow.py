...
class TestE2EDocumentFlow:
    ...
    @pytest.mark.asyncio
    async def test_complete_simple_document_processing_flow(self, mock_db_session, sample_pdf_path, mock_document):
    ...
    @pytest.mark.asyncio
    async def test_complete_complex_document_processing_flow(self, mock_db_session, sample_pdf_path, mock_document):
    ...
    @pytest.mark.asyncio
    async def test_document_classification_workflow(self, mock_db_session, sample_pdf_path):
        """Test document classification workflow when type is not set."""
        # Arrange
        mock_document = Document(
            id=uuid.uuid4(),
            filename="unclassified_document.pdf",
            document_type=None,  # Not yet classified
            status=DocumentStatus.UPLOADED,
            client_id=uuid.uuid4(),
            content_hash="unclassified_hash",
            file_size=1024,
            ...
            with patch('app.workers.llama_index_processor.get_document_preprocessor') as mock_preprocessor:
                with patch('app.workers.llama_index_processor.get_security_validator') as mock_validator:
                    ...
                    with patch.object(LlamaIndexProcessor, '_extract_simple_text', return_value="Sample text"):
                        ...
                        assert chunk.chunk_metadata["chunk_position"] == 0
                        assert chunk.chunk_metadata["total_chunks"] == 1
                        assert "document_id" in chunk.chunk_metadata
                        assert "filename" in chunk.chunk_metadata

    @pytest.mark.asyncio
    async def test_vector_storage_integration(self, mock_db_session, sample_pdf_path, mock_document):
        """Test integration with vector storage."""
        # This test would verify that chunks are properly stored in the vector database
        # Implementation depends on actual vector store integration
        pass

    @pytest.mark.asyncio
    async def test_concurrent_document_processing(self, mock_db_session, sample_pdf_path):
        """Test handling of concurrent document processing requests."""
        # This test would verify that multiple documents can be processed concurrently
