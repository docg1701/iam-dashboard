...
class TestClientAPI:
    ...
    def test_create_client_success(self, mock_time, mock_email, test_client: TestClient, auth_headers: dict, db: Session):
        ...
        created_client = db.query(Client).filter(Client.cpf == "123.456.789-09").first()
    ...
    def test_create_client_duplicate_cpf(self, mock_email, test_client: TestClient, auth_headers: dict, db: Session):
        ...
        existing_client = ClientFactory(cpf="123.456.789-09")
        ...
        clients_with_cpf = db.query(Client).filter(Client.cpf == "123.456.789-09").all()
...
// Example error responses
{
  "error": {
    ...
    "details": {
      "field": "cpf",
      "received": "123456789",
      "expected": "XXX.XXX.XXX-XX format"
    },
...
AGENT1_ERRORS = {
    "INVALID_CPF": "CPF format validation failed",
    "DUPLICATE_CPF": "Client with CPF already exists",
