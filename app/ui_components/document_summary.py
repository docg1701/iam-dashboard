...
class DocumentSummaryModal:
        ...
        with ui.column().classes("w-full p-4 flex-1 overflow-auto"):
            ...
            with ui.card().classes("w-full mb-6"):
                ...
                with ui.row().classes("w-full gap-6"):
                    with ui.column().classes("flex-1"):
                        ui.label(f"Nome: {self.document.filename}").classes(
                            "text-base mb-2"
                        )
                        ui.label(
                            f"Tamanho: {self.document.formatted_file_size}"
                        ).classes("text-base mb-2")
                        ui.label(
                            f"Tipo: {self.document.document_type.title()}"
                        ).classes("text-base mb-2")
                    ...
                    with ui.column().classes("flex-1"):
                        ...
                        ui.label(f"Processado em: {processed_at}").classes(
                            "text-base mb-2"
                        )
                        ...
                        if self.document.created_at and self.document.processed_at:
                            processing_time = (
                                self.document.processed_at - self.document.created_at
                            )
                            hours, remainder = divmod(
                                processing_time.total_seconds(), 3600
                            )
                            ...
                            ui.label(f"Tempo de processamento: {time_str}").classes(
                                "text-base mb-2"
                            )
            ...
            with ui.card().classes("w-full mb-6"):
                with ui.row().classes("w-full justify-between items-center mb-4"):
                    ...
                    self.chunks_count_label = ui.label("").classes(
                ...
                ) as self.loading_container:
                    ui.spinner(size="lg")
                    ui.label("Carregando conteúdo...").classes("ml-4 text-lg")

            # Statistics section
            with ui.card().classes("w-full"):
                ui.label("Estatísticas do Processamento").classes(
    ...
    def _create_footer(self) -> None:
        ...
        with ui.row().classes("w-full justify-end gap-2 p-4 border-t"):
            ...
            ui.button("Fechar", on_click=self.dialog.close).classes(
    ...
    async def _load_document_details(self) -> None:
        ...
        try:
            async for db_session in get_async_db():
                ...
                self.chunks = await chunk_repository.get_by_document_id(
                    self.document.id
                )

                # Update chunks count
                self.chunks_count_label.text = (
                    f"{len(self.chunks)} blocos de texto encontrados"
                )
            ...
            with self.loading_container:
                ...
                ui.label(f"Erro ao carregar detalhes: {str(e)}").classes(
                    "ml-4 text-lg text-red-600"
                )
    ...
    async def _display_chunks(self) -> None:
        ...
        if not self.chunks:
            with self.chunks_container:
                ui.label("Nenhum conteúdo extraído encontrado").classes(
            ...
            with self.chunks_container:
                with ui.card().classes("w-full mb-4"):
                    with ui.row().classes("w-full justify-between items-center mb-2"):
                        ...
                        ui.label(f"{char_count} caracteres").classes(
                            "text-sm text-gray-500"
                        )

                    # Content preview (first 500 characters)
                    content_preview = (
                        chunk.text[:500] if chunk.text else "Conteúdo vazio"
                    )
                    if len(chunk.text) > 500:
                        content_preview += "..."

                    with ui.expansion("Ver conteúdo", icon="visibility").classes(
                        "w-full"
                    ):
                            ...
                            if len(chunk.text) > 500:
                                with ui.row().classes("w-full justify-center mt-4"):
                                    ui.button(
                                        ...
                                        on_click=lambda c=chunk: self._show_full_content(
                                            c
                                        ),
                                    ).classes(
                                        "bg-blue-500 text-white px-4 py-2 rounded text-sm"
                                    )

                    # Metadata if available
                    if chunk.chunk_metadata:
                        with ui.expansion("Metadados", icon="info").classes(
                            "w-full mt-2"
                        ):
                            with ui.column().classes("w-full p-4 bg-gray-50 rounded"):
                                ...
                                if "chunk_position" in metadata:
                                    ui.label(
                                        f"Posição: {metadata['chunk_position']}"
                                    ).classes("text-sm mb-1")
                                if "total_chunks" in metadata:
                                    ui.label(
                                        f"Total de chunks: {metadata['total_chunks']}"
                                    ).classes("text-sm mb-1")
                                if "char_count" in metadata:
                                    ui.label(
                                        f"Caracteres: {metadata['char_count']:,}"
                                    ).classes("text-sm mb-1")
                                ...
                                if "document_id" in metadata:
                                    ui.label(
                                        f"ID do documento: {str(metadata['document_id'])[:8]}..."
                                    ).classes("text-sm mb-1")
                                ...
                                if "file_size" in metadata:
                                    ui.label(
                                        f"Tamanho: {metadata['file_size']:,} bytes"
                                    ).classes("text-sm mb-1")
                                if "client_id" in metadata:
                                    ui.label(
                                        f"Cliente ID: {str(metadata['client_id'])[:8]}..."
                                    ).classes("text-sm mb-1")
                                ...
                                if additional_metadata:
                                    ...
                                    ui.label("Metadados adicionais:").classes(
                        ...
                        with ui.expansion("Metadados", icon="info").classes(
                            ...
                            with ui.column().classes("w-full p-4 bg-gray-50 rounded"):
                                ui.label(
                                    "Nenhum metadado disponível para este bloco"
        ...
        with ui.dialog() as content_dialog:
            with ui.card().style("width: 90vw; max-width: 800px; max-height: 80vh"):
                with ui.row().classes(
                    ...
                    ui.button("", icon="close", on_click=content_dialog.close).props(
                        "flat round"
                    )

                with ui.column().classes("w-full p-4 overflow-auto"):
                    ui.label(chunk.text or "Conteúdo vazio").classes(
                        "text-sm whitespace-pre-wrap"
                    )
        ...
        for i, chunk in enumerate(self.chunks):
            with self.chunks_container:
                with ui.card().classes("w-full mb-4"):
                    ...
                    content_preview = (
                        chunk.text[:200] if chunk.text else "Conteúdo vazio"
                    )
        ...
        with self.stats_container:
            ...
            with ui.row().classes("w-full gap-8 mb-6"):
                ...
                if self.chunks:
                    ...
                    with ui.column().classes("text-center"):
                        ui.label(f"{avg_chunk_size:,}").classes(
            ...
            with ui.row().classes("w-full gap-6"):
                with ui.column().classes("flex-1"):
                    ui.label("Informações do Processamento:").classes(
                    ...
                    try:
                        # Status
                        status_text = (
                            self.document.status.value
                            if hasattr(self.document.status, "value")
                            else str(self.document.status)
                        )
                        ui.label(f"• Status: {status_text}").classes("text-sm mb-1")

                        # Document type
                        doc_type = (
                            str(self.document.document_type).title()
                        ...
                        if self.document.content_hash:
                            hash_preview = (
                                self.document.content_hash[:16] + "..."
                                if len(self.document.content_hash) > 16
                                else self.document.content_hash
                            )
                            ui.label(f"• Hash SHA256: {hash_preview}").classes(
                                "text-sm mb-1"
                            )
                        ...
                        if self.document.processed_at and self.document.created_at:
                            ...
                            ui.label(f"• Tempo total: {processing_duration}").classes(
                                "text-sm mb-1"
                            )
                        ...
                        if hasattr(self.document, "task_id") and self.document.task_id:
                            ui.label(
                                f"• Task ID: {self.document.task_id[:8]}..."
                            ).classes("text-sm mb-1")

                    except Exception as e:
                        ui.label(f"• Erro ao carregar informações: {str(e)}").classes(
                            "text-sm mb-1 text-red-500"
                        )

                with ui.column().classes("flex-1"):
                    ui.label("Estatísticas por Bloco:").classes(
                        "text-lg font-semibold mb-2"
                    )
                    ...
                    try:
                        if self.chunks and len(self.chunks) > 0:
                            chunk_lengths = [
                                len(chunk.text) for chunk in self.chunks if chunk.text
                            ]
                            if chunk_lengths:
                                min_chars = min(chunk_lengths)
                                max_chars = max(chunk_lengths)
                                ui.label(
                                    f"• Menor bloco: {min_chars:,} caracteres"
                                ).classes("text-sm mb-1")
                                ui.label(
                                    f"• Maior bloco: {max_chars:,} caracteres"
                                ).classes("text-sm mb-1")
                                ui.label(
                                    f"• Média: {avg_chunk_size:,} caracteres"
                                ).classes("text-sm mb-1")
                                ...
                                ui.label(
                                    f"• Embeddings gerados: {total_embeddings}/{len(self.chunks)}"
                                ).classes("text-sm mb-1")
                            else:
                                ui.label(
                                    "• Nenhum texto encontrado nos blocos"
                                ).classes("text-sm mb-1 text-gray-500")
                        else:
                            ui.label("• Nenhum bloco disponível").classes(
                                "text-sm mb-1 text-gray-500"
                            )

                    except Exception as e:
                        ui.label(f"• Erro ao calcular estatísticas: {str(e)}").classes(
                            "text-sm mb-1 text-red-500"
                        )
