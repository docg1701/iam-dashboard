...
import React, { createContext, useContext, useCallback, useReducer, ReactNode, useEffect } from 'react';
...
interface AppError {
  ...
  code?: string;
  statusCode?: number;
  category: 'auth' | 'network' | 'validation' | 'permission' | 'system' | 'unknown';
...
interface ErrorContextType {
  ...
  addAuthError: (message: string, context?: Record<string, any>) => string;
  addNetworkError: (message: string, statusCode?: number, context?: Record<string, any>) => string;
  addValidationError: (message: string, context?: Record<string, any>) => string;
  addPermissionError: (message: string, context?: Record<string, any>) => string;
  addSystemError: (message: string, context?: Record<string, any>) => string;
...
export const ErrorProvider: React.FC<ErrorProviderProps> = ({
  ...
  const addError = useCallback((error: Omit<AppError, 'id' | 'timestamp'>): string => {
    ...
    if (enableConsoleLogging) {
      const logLevel = fullError.severity === 'critical' || fullError.severity === 'high' ? 'error' : 'warn';
      console.group(`🚨 ${fullError.category.toUpperCase()} Error [${fullError.severity.toUpperCase()}]`);
      console[logLevel]('Message:', fullError.message);
      console[logLevel]('Code:', fullError.code || 'N/A');
      console[logLevel]('Status:', fullError.statusCode || 'N/A');
      console[logLevel]('Context:', fullError.context || {});
      if (fullError.stack) {
        console[logLevel]('Stack:', fullError.stack);
      }
      console.groupEnd();
    }
  ...
  // Helper methods for common error types
  const addAuthError = useCallback((message: string, context?: Record<string, any>): string => {
    return addError({
      message,
      category: 'auth',
      severity: 'medium',
  ...
  const addNetworkError = useCallback((message: string, statusCode?: number, context?: Record<string, any>): string => {
  ...
  const addValidationError = useCallback((message: string, context?: Record<string, any>): string => {
    return addError({
      message,
      category: 'validation',
      severity: 'low',
  ...
  const addPermissionError = useCallback((message: string, context?: Record<string, any>): string => {
    return addError({
      message,
      category: 'permission',
      severity: 'medium',
  ...
  const addSystemError = useCallback((message: string, context?: Record<string, any>): string => {
    return addError({
      message,
      category: 'system',
      severity: 'high',
  ...
  const getErrorsByCategory = useCallback((category: AppError['category']): AppError[] => {
    return state.errors.filter(error => error.category === category);
  }, [state.errors]);

  const hasErrorsOfSeverity = useCallback((severity: AppError['severity']): boolean => {
  ...
  useEffect(() => {
    const cleanup = setInterval(() => {
      ...
      const recentErrors = state.errors.filter(error => error.timestamp > fifteenMinutesAgo);
...
export const useErrorHandler = () => {
  ...
  return useCallback((error: Error, category: AppError['category'] = 'unknown', context?: Record<string, any>) => {
    addError({
      message: error.message,
      category,
      severity: category === 'auth' || category === 'permission' ? 'medium' : 'high',
      stack: error.stack,
      context,
    });
  }, [addError]);
