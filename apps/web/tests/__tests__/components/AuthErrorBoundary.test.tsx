...
} from '@/components/errors/AuthErrorBoundary'
...
// Typed interfaces for test errors
...
interface TfaError extends Error {
  statusCode: number
  code: string
}
...
const ErrorThrowingComponent = ({
  ...
  // Throw error synchronously during render to avoid useEffect loops
  switch (errorType) {
    case 'auth':
    ...
    case 'network':
      ...
      throw networkError
    case 'server':
      const serverError = new Error('Internal server error') as ServerError
      serverError.statusCode = 500
      throw serverError
    case '2fa':
      const tfaError = new Error('Invalid 2FA code') as TfaError
...
const TestWrapper = ({ children }: { children: React.ReactNode }) => (
  <ErrorProvider enableGlobalErrorHandler={false}>{children}</ErrorProvider>
)
...
describe('AuthErrorBoundary', () => {
  ...
  describe('Error Categorization and Display', () => {
    it('should categorize and display authentication errors correctly', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Credenciais inválidas ou sessão expirada/)
        ).toBeInTheDocument()
    ...
    it('should categorize and display network errors correctly', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Erro de Autenticação')).toBeInTheDocument()
        expect(
          screen.getByText(/Problema de conexão com o servidor/)
        ).toBeInTheDocument()
    ...
    it('should categorize and display 2FA errors correctly', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Erro de Autenticação')).toBeInTheDocument()
        expect(
          screen.getByText(/Código de autenticação de dois fatores inválido/)
        ).toBeInTheDocument()
        expect(screen.getByText('Tentar Novamente')).toBeInTheDocument()
      })
    })
  ...
  describe('Error Recovery Actions', () => {
    it('should handle retry action for retryable errors', async () => {
      ...
      // Should retry and show error again (since component always throws)
      await waitFor(() => {
        expect(screen.getByText('Erro de Autenticação')).toBeInTheDocument()
      })
    })
  ...
  describe('Development Mode Features', () => {
    it('should show technical details in development mode', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Detalhes Técnicos (Desenvolvimento)')
        ).toBeInTheDocument()
      })
  ...
  describe('Custom Error Handler', () => {
    it('should call custom error handler when provided', async () => {
      ...
      const [error, errorInfo] = mockErrorHandler.mock.calls[0] as [
        Error,
