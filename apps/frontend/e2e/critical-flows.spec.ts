...
test.describe('Critical User Flows', () => {
  ...
  test('complete user journey - login to dashboard', async ({ page }) => {
    ...
    if (currentUrl.includes('2fa') || await page.locator('input[name="code"]').isVisible()) {
      // Handle 2FA flow
  ...
  test('client management workflow', async ({ page }) => {
    ...
    const newClientButton = page.locator('button:has-text("Adicionar"), button:has-text("Novo"), button:has-text("Criar")')
    ...
    await expect(page.locator('table, .client-list, [data-testid="clients-table"]')).toBeVisible()
  })
  ...
  test('user management and permissions flow', async ({ page }) => {
    ...
    await page.click('a[href*="users"], a[href*="usuarios"], button:has-text("Usuários")')
    await page.waitForLoadState('networkidle')
    ...
    const permissionLink = page.locator('a[href*="permissions"], a[href*="permissoes"], button:has-text("Permissões")')
    if (await permissionLink.isVisible()) {
      await permissionLink.click()
      await page.waitForLoadState('networkidle')
      
      // Verify permission matrix or management interface
      await expect(page.locator('table, .permission-matrix, [data-testid="permission-matrix"]')).toBeVisible()
    }
  })
  ...
  test('responsive design - mobile view', async ({ page }) => {
    ...
    const hamburgerMenu = page.locator('button[aria-label*="menu"], .hamburger, [data-testid="mobile-menu"]')
    if (await hamburgerMenu.isVisible()) {
      await hamburgerMenu.click()
      
      // Verify mobile menu opened
      await expect(page.locator('nav, .mobile-nav, [data-testid="mobile-navigation"]')).toBeVisible()
    }
