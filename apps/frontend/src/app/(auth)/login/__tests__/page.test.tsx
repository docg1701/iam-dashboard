...
describe("LoginPage", () => {
  ...
  describe("Page Structure and Layout", () => {
    test("renders login page with proper structure and branding", () => {
      ...
      expect(
        screen.getByText("Sistema de Gestão de Identidade e Acesso"),
      ).toBeInTheDocument();

      // Check login card structure
      expect(screen.getByText("Entrar na sua conta")).toBeInTheDocument();
      expect(
        screen.getByText("Digite suas credenciais para acessar o sistema"),
      ).toBeInTheDocument();
      ...
      expect(
        screen.queryByText(/verificação em duas etapas/i),
  ...
  describe("Login Form Integration and Flow", () => {
    ...
    test("handles successful login with 2FA requirement", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/verificação em duas etapas/i),
        ).toBeInTheDocument();
      ...
      expect(
        screen.queryByPlaceholderText(/seu@email\.com/i),
      ).not.toBeInTheDocument();
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/obrigatório/i) ||
            ...
            screen.getByText(/Email é obrigatório/i),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Two-Factor Authentication Flow", () => {
    ...
    test("handles successful 2FA verification", async () => {
      ...
      const digitInputs = codeInputs.filter(
