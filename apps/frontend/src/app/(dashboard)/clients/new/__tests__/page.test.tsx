...
describe("NewClientPage", () => {
  ...
  describe("Page Structure and Layout", () => {
    test("renders page header with proper structure", () => {
      ...
      expect(
        screen.getByRole("heading", { level: 1, name: /novo cliente/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/registre um novo cliente no sistema/i),
      ).toBeInTheDocument();
    });
    ...
    test("renders navigation back button", () => {
      ...
      expect(
        screen.getByRole("button", { name: /voltar para clientes/i }),
      ).toBeInTheDocument();
    });
    ...
    test("renders form section initially", () => {
      ...
      expect(
        screen.getByRole("heading", {
          level: 2,
          name: /informações do cliente/i,
        }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/preencha os campos abaixo/i),
      ).toBeInTheDocument();
    });
  ...
  describe("Form Interaction and Validation", () => {
    test("renders form fields correctly", () => {
      ...
      expect(
        screen.getByPlaceholderText(/joão silva santos/i),
      ).toBeInTheDocument();
      expect(
        screen.getByPlaceholderText(/123\.456\.789-12/i),
      ).toBeInTheDocument();
      expect(
    ...
    test("allows user to fill form fields", async () => {
      ...
      expect((cpfInput as HTMLInputElement).value).toMatch(
        /\d{3}\.\d{3}\.\d{3}-\d{2}/,
      );
  ...
  describe("Successful Client Creation", () => {
    ...
    test("shows success actions after client creation", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("button", { name: /ver cliente/i }),
        ).toBeInTheDocument();
      });

      expect(
        screen.getByRole("button", { name: /criar outro cliente/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("Content and Instructions", () => {
    test("displays comprehensive help information", () => {
      ...
      expect(
        screen.getByText(/informações podem ser editadas/i),
      ).toBeInTheDocument();
  ...
  describe("Accessibility", () => {
    ...
    test("provides informative descriptions", () => {
      ...
      expect(
        screen.getByText(/todos os campos marcados são obrigatórios/i),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Visual and Layout", () => {
    ...
    test("shows appropriate icons", () => {
      ...
      const userPlusIcons = document.querySelectorAll(
        '[class*="lucide-user-plus"]',
      );
