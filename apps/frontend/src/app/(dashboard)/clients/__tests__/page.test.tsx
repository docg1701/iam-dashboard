...
describe('ClientsPage - Comprehensive Coverage', () => {
  ...
  describe('Page Structure and Layout', () => {
    ...
    test('renders with proper responsive design classes', () => {
      ...
      // Main container
      expect(document.querySelector('.container.mx-auto.px-4.py-8.max-w-6xl')).toBeInTheDocument()
      
      // Header section
      expect(document.querySelector('.mb-6')).toBeInTheDocument()
      expect(document.querySelector('.flex.items-center.justify-between.mb-4')).toBeInTheDocument()
  ...
  describe('Empty State Display', () => {
    test('shows complete empty state structure with proper Card styling', () => {
      ...
      expect(screen.getByText(/comece criando o primeiro cliente/i)).toBeInTheDocument()
      
      // Check flex column layout
      const flexContainer = document.querySelector('.flex.flex-col.items-center.gap-4')
    ...
    test('displays empty state icon with proper styling', () => {
      ...
      // Icon container
      const iconContainer = document.querySelector('.p-3.bg-muted.rounded-full')
      expect(iconContainer).toBeInTheDocument()
      
      // Users icon in empty state
      const emptyStateIcon = iconContainer?.querySelector('[class*="lucide-users"]')
    ...
    test('shows empty state call-to-action with proper button styling', () => {
      ...
      const ctaButton = screen.getByRole('button', { name: /criar primeiro cliente/i })
    ...
    test('displays complete educational information card with proper styling', () => {
      ...
      const infoCard = document.querySelector('.p-4.mt-6.bg-blue-50.border-blue-200')
      ...
      // Info list
      const infoList = document.querySelector('.text-sm.text-blue-700.space-y-1')
  ...
  describe('Navigation Structure', () => {
    test('has proper semantic structure', () => {
      ...
      // Section headings
      expect(screen.getByRole('heading', { level: 3 })).toHaveTextContent(/nenhum cliente encontrado/i)
      expect(screen.getByRole('heading', { level: 4 })).toHaveTextContent(/sobre o sistema de clientes/i)
    })
