...
const PermissionStatistics: React.FC<{
  ...
  const stats = useMemo((): PermissionStats => {
    ...
    const usersWithAccess = users.filter((user) => {
      ...
      return (
        userPerms &&
        Object.values(userPerms.permissions).some((agentPerms) =>
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            {stats.activeUsers} ativos, {stats.totalUsers - stats.activeUsers}{" "}
            inativos
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            {stats.totalUsers > 0
              ? Math.round((stats.usersWithAccess / stats.totalUsers) * 100)
              : 0}
            % do total
          </p>
        </CardContent>
...
const QuickActionsToolbar: React.FC<{
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <Button variant="default" size="sm" onClick={onUserCreate}>
        <Plus className="h-4 w-4 mr-2" />
        Novo Usuário
      </Button>

      <Button variant="outline" size="sm" onClick={onTemplateManagement}>
        <Settings className="h-4 w-4 mr-2" />
        Templates
      </Button>

      <Button variant="outline" size="sm" onClick={onExportPermissions}>
        <Download className="h-4 w-4 mr-2" />
        Exportar
      </Button>

      <Button variant="outline" size="sm" onClick={onImportPermissions}>
        <Upload className="h-4 w-4 mr-2" />
        Importar
      </Button>

      {selectedUsers.length > 0 && (
        <Button variant="secondary" size="sm" onClick={onBulkOperation}>
...
export default function AdminPermissionsPage() {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      if (filters.permissionStatus !== "all") {
        ...
        if (filters.permissionStatus === "with_access" && !hasPermissions)
          return false;
        if (filters.permissionStatus === "no_access" && hasPermissions)
          return false;
      }
  ...
  const handleBulkOperation = useCallback(() => {
    if (selectedUsers.length === 0) {
      toast({
        ...
        description:
          "Selecione pelo menos um usuário para executar operações em lote.",
        variant: "error",
      });
      return;
    }
  ...
  const handleBulkOperationComplete = useCallback(
    (result: BulkPermissionAssignResponse) => {
          ...
          <>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Filtre usuários por diferentes critérios para facilitar o
                  gerenciamento
                </CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <TabsContent value="matrix" className="space-y-4">
                <PermissionMatrix
                  ...
                  onBulkAction={(userIds, action) => {
                    const selectedUsersForBulk = filteredUsers.filter((u) =>
