...
export const PermissionTemplateUpdateSchema = z.object({
  ...
  description: z.string().optional(),
  permissions: z.record(z.nativeEnum(AgentName), PermissionActionsSchema).optional(),
})
...
export interface UseUserPermissionsActions {
  hasPermission: (agent: AgentName, operation: keyof PermissionActions) => boolean
...
export type PartialAgentPermissions = Partial<Record<AgentName, Partial<PermissionActions>>>
...
export const isValidPermissionOperation = (value: string): value is PermissionOperation => {
...
export const getPermissionLevel = (permissions: PermissionActions): PermissionLevel => {
...
export const getPermissionsForLevel = (level: PermissionLevel): PermissionActions => {
  switch (level) {
