...
export function usePermissionUpdates(
  ...
  const handlePermissionUpdate = useCallback(
    (event: PermissionUpdateEvent) => {
      ...
      if (enableToastNotifications) {
        const agentName =
          AGENT_DISPLAY_NAMES[event.agent_name] || event.agent_name;
  ...
  const connect = useCallback(() => {
    if (!user?.user_id || !token) {
      console.warn(
        "Cannot connect to permission updates: user not authenticated",
      );
      return;
    }
    ...
    if (wsManagerRef.current.isConnecting) {
      console.log(
        "Permission updates WebSocket connection already in progress",
      );
      return;
    }
    ...
    try {
      ...
      ws.onclose = (event) => {
        console.log(
          "Permission updates WebSocket closed:",
          event.code,
          event.reason,
        );
        ...
        if (
          autoReconnect &&
          event.code !== 1000 &&
          wsManagerRef.current.reconnectAttempts < maxReconnectAttempts
        ) {
          ...
          if (enableToastNotifications) {
            toast({
              ...
              description:
                "Não foi possível reconectar às atualizações em tempo real.",
              variant: "error",
            });
          }
        }
