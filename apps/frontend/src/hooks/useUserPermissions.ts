...
const PERMISSION_QUERY_KEYS = {
  ...
  userAgent: (userId: string, agent: AgentName) =>
    ["permissions", "user", userId, "agent", agent] as const,
...
export function useUserPermissions(
  userId?: string,
): UseUserPermissionsState & UseUserPermissionsActions {
      ...
      try {
        ...
        permissionWebSocket.onmessage = (event) => {
          try {
            ...
            if (
              message.event.type === "permission_update" &&
  ...
  const getUserMatrix = useCallback((): UserPermissionMatrix | null => {
...
export function useAgentAccess(agent: AgentName, userId?: string) {
  const { hasAgentPermission, permissions, isLoading, error } =
...
export function usePermissionMutations(userId?: string) {
  ...
  const updatePermission = useMutation({
    mutationFn: ({
      ...
      console.log("Update permission requested for:", {
        targetUserId,
        agent,
        permissions,
      });
      throw new PermissionError(
        "Update permission implementation needed",
        "NOT_IMPLEMENTED",
      );
    },
    onSuccess: () => {
  ...
  const deletePermission = useMutation({
    mutationFn: (agent: AgentName) => {
      ...
      throw new PermissionError(
        "Delete permission implementation needed",
        "NOT_IMPLEMENTED",
      );
    },
    onSuccess: () => {
...
export function usePermissionTemplates() {
  ...
  const applyTemplate = useMutation({
    mutationFn: ({
      ...
      return PermissionAPI.Template.applyTemplateToUser(
        templateId,
        userId,
        changeReason,
      );
