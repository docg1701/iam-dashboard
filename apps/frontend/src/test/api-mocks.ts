...
export const createMockPermissions = (
  overrides?: Partial<PermissionActions>,
): PermissionActions => ({
  create: false,
  read: true,
  update: false,
 ...
 */
export const createMockUserPermissionsResponse = (
  userId?: string,
  permissions?: UserAgentPermission[],
) => ({
  user_id: userId || "123e4567-e89b-12d3-a456-426614174000",
  permissions: permissions || [
    createMockUserAgentPermission(userId, AgentName.CLIENT_MANAGEMENT, {
      create: true,
      read: true,
      update: true,
    }),
    createMockUserAgentPermission(userId, AgentName.PDF_PROCESSING, {
      read: true,
    }),
  ],
  last_updated: "2024-01-15T14:30:00Z",
 ...
 */
export const createMockTemplatesResponse = (
  templates?: PermissionTemplate[],
) => ({
  templates: templates || [
    createMockPermissionTemplate({
      template_name: "Usuário Básico",
 ...
 */
export const createMockAuditLogsResponse = (
  ...
  logs: logs || [
    ...
    createMockAuditLog({
      ...
      old_permissions: createMockPermissions({
        create: true,
        read: true,
        update: true,
      }),
  ...
  errors: errorCount
    ? [
        { user_id: "user-error-1", error: "Usuário não encontrado" },
        {
          ...
          error: "Permissão inválida para este usuário",
        },
      ]
    : [],
});
...
export const setupUserAPITest = (options?: {
  ...
  const response = options?.customResponse || {
    user: options?.user || createMockAdminUser(),
  };
...
export const setupTemplateAPITest = (options?: {
  ...
  const response =
    options?.customResponse || createMockTemplatesResponse(options?.templates);
...
export const setupPermissionMatrixTest = (options?: {
  ...
  users.forEach((user, index) => {
    if (!options?.shouldFailPermissions) {
      const userMatrix =
        options?.userPermissions?.[user.user_id] ||
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve(
            createMockUserPermissionsResponse(user.user_id, [
              createMockUserAgentPermission(
                user.user_id,
                AgentName.CLIENT_MANAGEMENT,
                userMatrix.permissions[AgentName.CLIENT_MANAGEMENT],
              ),
              createMockUserAgentPermission(
                user.user_id,
                AgentName.PDF_PROCESSING,
                userMatrix.permissions[AgentName.PDF_PROCESSING],
              ),
              createMockUserAgentPermission(
                user.user_id,
                AgentName.REPORTS_ANALYSIS,
                userMatrix.permissions[AgentName.REPORTS_ANALYSIS],
              ),
              createMockUserAgentPermission(
                user.user_id,
                AgentName.AUDIO_RECORDING,
                userMatrix.permissions[AgentName.AUDIO_RECORDING],
              ),
            ]),
          ),
      } as Response);
      ...
      mockFetch.mockRejectedValueOnce(
        new Error(`Failed to fetch permissions for user ${user.user_id}`),
      );
    }
  });
};
...
export const QueryKeys = {
  ...
  userAgentPermission: (userId: string, agent: AgentName) => [
    "user-permissions",
    userId,
    "agents",
    agent,
  ...
  permissionTemplate: (templateId: string) => [
...
export const TestDataPresets = {
  ...
  userWithNoPermissions: {
    ...
    permissions: createMockUserPermissionMatrix(
      undefined,
      DEFAULT_AGENT_PERMISSIONS,
    ),
  },
