...
const setupDefaultFetchMocks = () => {
  vi.mocked((global as any).fetch).mockImplementation(
    (url: any, options?: any) => {
      ...
      if (urlStr.includes("/permissions")) {
        if (
          (urlStr.includes("/users/") || urlStr.includes("/user/")) &&
          method === "GET"
        ) {
          ...
          const requestedUserId = userIdMatch
            ? userIdMatch[1]
            : "test-user-123";

          // Provide full permissions for admin users and basic permissions for regular users
          const isAdmin =
            requestedUserId.includes("admin") || urlStr.includes("admin");
