...
const getBirthDateField = () =>
  document.querySelector('input[name="birth_date"]') as HTMLInputElement;
const getNotesField = () =>
...
describe("ClientRegistrationForm - Integration Tests", () => {
  ...
  describe("Complete Client Registration Flow", () => {
    it("should complete full client registration workflow", async () => {
      ...
      await user.type(
        notesField,
        "Cliente preferencial com histórico excelente",
      );
    ...
    it("should handle API validation errors correctly", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/client with this cpf already exists/i),
        ).toBeInTheDocument();
      });
