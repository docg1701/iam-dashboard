...
describe('UserEditForm', () => {
  ...
  describe('Form Validation', () => {
    ...
    it('should show validation error for long name', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/nome deve ter no máximo 255 caracteres/i)).toBeInTheDocument()
      })
    })
  })
  ...
  describe('Role Selection', () => {
    ...
    it('should display role change warning', () => {
      ...
      expect(screen.getByText(/alterações no role afetam as permissões/i)).toBeInTheDocument()
    })
  })
  ...
  describe('Form State Management', () => {
    it('should maintain form state across re-renders', async () => {
      ...
      rerender(
        <TestWrapper>
          <UserEditForm user={mockUser} onSuccess={mockOnSuccess} onCancel={mockOnCancel} />
        </TestWrapper>
      )

      expect(screen.getByDisplayValue('João Silva Updated')).toBeInTheDocument()
    })

    it('should reset form when user prop changes', () => {
      const { rerender } = renderComponent()

      const newUser = { ...mockUser, user_id: '456', full_name: 'Maria Santos' }
      rerender(
        <TestWrapper>
          <UserEditForm user={newUser} onSuccess={mockOnSuccess} onCancel={mockOnCancel} />
        </TestWrapper>
      )
