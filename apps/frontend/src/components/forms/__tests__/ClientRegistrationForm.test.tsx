...
describe("ClientRegistrationForm", () => {
  ...
  it("should validate name with invalid characters", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          /nome deve conter apenas letras, espaços, hífens e apostrofes/i,
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should validate CPF format correctly", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/cpf deve estar no formato XXX\.XXX\.XXX-XX/i),
      ).toBeInTheDocument();
    });
  ...
  it("should validate minimum age requirement (13 years)", async () => {
    ...
    await waitFor(
      () => {
        const ageError = screen.queryByText(
          /idade deve estar entre 13 e 120 anos/i,
        );
  ...
  it("should validate notes maximum length", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/notas não podem exceder 1000 caracteres/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should handle API errors correctly", async () => {
    ...
    renderWithProviders(
      <ClientRegistrationForm onSubmit={mockOnSubmit} onError={mockOnError} />,
    );
  ...
  it("should display helpful field descriptions", () => {
    ...
    expect(
      screen.getByText(
        /nome completo do cliente conforme documento de identidade/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/cpf no formato XXX\.XXX\.XXX-XX \(apenas números\)/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/cliente deve ter pelo menos 13 anos de idade/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /notas ou informações adicionais relevantes \(máximo 1000 caracteres\)/i,
      ),
    ).toBeInTheDocument();
  });
