...
describe('PasswordStrengthIndicator', () => {
  describe('Basic rendering', () => {
    ...
    it('should hide details when showDetails is false', () => {
      ...
      expect(screen.queryByText('Pelo menos 8 caracteres')).not.toBeInTheDocument()
    })
  ...
  describe('Password complexity validation', () => {
    it('should validate minimum length requirement', () => {
      ...
      expect(lengthRequirement.closest('li')).toHaveClass('text-muted-foreground')
    ...
    it('should validate uppercase letter requirement', () => {
      ...
      const uppercaseRequirement = screen.getByText('Pelo menos 1 letra maiúscula')
      expect(uppercaseRequirement.closest('li')).toHaveClass('text-muted-foreground')
    })
    ...
    it('should validate lowercase letter requirement', () => {
      ...
      const lowercaseRequirement = screen.getByText('Pelo menos 1 letra minúscula')
      expect(lowercaseRequirement.closest('li')).toHaveClass('text-muted-foreground')
    })
    ...
    it('should validate number requirement', () => {
      ...
      expect(numberRequirement.closest('li')).toHaveClass('text-muted-foreground')
    })
    ...
    it('should validate special character requirement', () => {
      ...
      const specialCharRequirement = screen.getByText('Pelo menos 1 caractere especial')
      expect(specialCharRequirement.closest('li')).toHaveClass('text-muted-foreground')
    })
    ...
    it('should show check mark when special character requirement is met', () => {
      ...
      const specialCharRequirement = screen.getByText('Pelo menos 1 caractere especial')
      expect(specialCharRequirement.closest('li')).toHaveClass('text-green-600')
    })
  })
  ...
  describe('Visual progress indicators', () => {
    ...
    it('should show correct number of filled bars for score 2', () => {
      const { container } = render(<PasswordStrengthIndicator password="abcABC" />)
    ...
    it('should show correct number of filled bars for score 3', () => {
      const { container } = render(<PasswordStrengthIndicator password="abcdefghA" />)
    ...
    it('should show correct number of filled bars for score 4', () => {
      const { container } = render(<PasswordStrengthIndicator password="abcdefghA1" />)
  ...
  describe('Real-time validation feedback', () => {
    ...
    it('should show weak strength for partial password', async () => {
      ...
      expect(screen.getByText('Fraca')).toBeInTheDocument()
      expect(screen.getByText('Pelo menos 8 caracteres').closest('li')).toHaveClass('text-muted-foreground')
    })
    ...
    it('should progressively show improvements as requirements are met', async () => {
      ...
      expect(screen.getByText('Pelo menos 8 caracteres').closest('li')).toHaveClass('text-green-600')
      expect(screen.getByText('Regular')).toBeInTheDocument()
      
      // Step 2: Add uppercase
      await user.clear(input)
      await user.type(input, 'passwordP')
      expect(screen.getByText('Pelo menos 1 letra maiúscula').closest('li')).toHaveClass('text-green-600')
      expect(screen.getByText('Boa')).toBeInTheDocument()
      
      // Step 3: Add number
      await user.clear(input)
      await user.type(input, 'passwordP1')
      expect(screen.getByText('Pelo menos 1 número').closest('li')).toHaveClass('text-green-600')
      expect(screen.getByText('Forte')).toBeInTheDocument()
      
      // Step 4: Add special character
      await user.clear(input)
      await user.type(input, 'passwordP1!')
      expect(screen.getByText('Pelo menos 1 caractere especial').closest('li')).toHaveClass('text-green-600')
      expect(screen.getByText('Muito Forte')).toBeInTheDocument()
    })
  })
  ...
  describe('Accessibility features', () => {
    ...
    it('should use appropriate color contrast for different states', () => {
      ...
      expect(unmetRequirement.closest('li')).toHaveClass('text-muted-foreground')
    })
  ...
  describe('Complex password scenarios', () => {
    ...
    it('should handle passwords with Unicode characters', () => {
      ...
      expect(screen.getByText('Muito Forte')).toBeInTheDocument()
      expect(screen.getByText('Pelo menos 1 letra minúscula').closest('li')).toHaveClass('text-green-600')
    })
    ...
    it('should handle passwords with only special characters', () => {
      ...
      expect(screen.getByText('Pelo menos 8 caracteres').closest('li')).toHaveClass('text-green-600')
      expect(screen.getByText('Pelo menos 1 caractere especial').closest('li')).toHaveClass('text-green-600')
      expect(screen.getByText('Pelo menos 1 letra minúscula').closest('li')).toHaveClass('text-muted-foreground')
    })

    it('should cap score at maximum of 5', () => {
      const { container } = render(<PasswordStrengthIndicator password="VeryLongPasswordWithAllRequirements123!@#" />)
