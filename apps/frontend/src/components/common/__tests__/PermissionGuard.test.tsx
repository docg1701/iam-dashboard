  ...
  describe("Admin User Permissions", () => {
    ...
    it.skip("should deny admin access to restricted operations", async () => {
      ...
      const adminPermissions = [
        ...
        ), // EXPLICITLY NO delete permission
        createMockUserAgentPermission(
          limitedAdminUser.user_id,
          AgentName.REPORTS_ANALYSIS,
    ...
    it("should deny user access to create/update/delete operations", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(
              "Você não tem permissão para criar no agente Gestão de Clientes.",
            ),
          ).toBeInTheDocument();
          expect(
            screen.queryByText("Should Not See This"),
    ...
    it("should deny user access to restricted agents", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(
              "Você não tem permissão para visualizar no agente Relatórios e Análises.",
            ),
          ).toBeInTheDocument();
          expect(
            screen.queryByText("Should Not See This"),
  ...
  describe("Loading States", () => {
    ...
    beforeEach(async () => {
      await act(async () => {
        ...
        const permissionResponse = createMockUserPermissionsResponse(
          loadingUser.user_id,
          adminPermissions,
        );
  ...
  describe("Error Handling", () => {
    ...
    it("should show error message when permission check fails", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Erro ao verificar permissões/),
          ).toBeInTheDocument();
          expect(
    ...
    it("should deny access when user is not authenticated", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(
              "Você não tem permissão para visualizar no agente Gestão de Clientes.",
            ),
          ).toBeInTheDocument();
          expect(
            screen.queryByText("Should Not See This"),
