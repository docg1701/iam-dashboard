...
import { PermissionGuard, CreatePermissionGuard, UpdatePermissionGuard, DeletePermissionGuard } from '@/components/common/PermissionGuard'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
const TemplateForm: React.FC<{
  ...
  const handlePermissionChange = useCallback((agent: AgentName, level: PermissionLevel) => {
      ...
      <div className="space-y-4">
        <div>
          ...
          <Input
            ...
            onChange={(e) => onChange({ ...formData, template_name: e.target.value })}
        ...
        {mode === 'create' && (
          <div className="flex items-center space-x-2">
            <input
              ...
              onChange={(e) => onChange({ ...formData, is_system_template: e.target.checked })}
              disabled={isLoading}
            ...
            return (
              <Card key={agentValue}>
                ...
                <CardContent>
                  <Select
                    value={currentLevel}
                    onValueChange={(level) => handlePermissionChange(agentValue, level as PermissionLevel)}
                    disabled={isReadOnly || isLoading}
                  >
                  ...
                  <div className="mt-3 text-xs text-muted-foreground">
                    <div className="grid grid-cols-2 gap-1">
                      {Object.entries(agentPermissions).map(([operation, allowed]) => (
                        <div key={operation} className={`flex items-center ${allowed ? 'text-green-600' : 'text-gray-400'}`}>
                          ...
                          {allowed ? <CheckCircle className="h-3 w-3 ml-1" /> : <X className="h-3 w-3 ml-1" />}
                        </div>
                      ))}
                    </div>
                  </div>
    ...
    <TableRow>
      ...
      <TableCell>
        <div className="flex flex-wrap gap-1">
          {Object.entries(permissionSummary).map(([level, count]) => (
            count > 0 && (
              <Badge 
                key={level} 
                variant="outline" 
    ...
    <PermissionGuard agent={AgentName.CLIENT_MANAGEMENT} operation="read">
      <main className={`space-y-6 ${className}`}>
        ...
        <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          <div>
            <h2 className="text-2xl font-bold tracking-tight">Templates de Permissão</h2>
            <p className="text-muted-foreground">
              Crie e gerencie templates reutilizáveis para facilitar a atribuição de permissões
            </p>
          </div>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center">
                ...
                <div className="ml-2">
                  <p className="text-sm font-medium leading-none">Total de Templates</p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center">
                ...
                <div className="ml-2">
                  <p className="text-sm font-medium leading-none">Templates do Sistema</p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center">
                ...
                <div className="ml-2">
                  <p className="text-sm font-medium leading-none">Templates Personalizados</p>
                  <p className="text-2xl font-bold">
                    {templates.filter(t => !t.is_system_template).length}
                  </p>
                </div>
        ...
        <Dialog open={dialogState.open} onOpenChange={(open) => !open && closeDialog()}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {dialogState.mode === 'create' && 'Crie um novo template de permissões para reutilizar'}
                {dialogState.mode === 'edit' && 'Modifique as configurações deste template'}
                {dialogState.mode === 'view' && 'Visualize as configurações deste template'}
                {dialogState.mode === 'delete' && 'Confirme a exclusão deste template'}
              </DialogDescription>
            </DialogHeader>
            ...
            {dialogState.mode === 'delete' ? (
              <div className="space-y-4">
                <Alert variant="destructive">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    Esta ação não pode ser desfeita. O template &quot;{dialogState.template?.template_name}&quot; 
