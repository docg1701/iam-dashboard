 ...
 */
const ProgressTracker: React.FC<{ progress: OperationProgress; users: User[] }> = ({ progress, users }) => {
 ...
 */
const TemplatePreview: React.FC<{ template: PermissionTemplate | null }> = ({ template }) => {
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-2 gap-2">
          {Object.entries(template.permissions).map(([agentName, permissions]) => {
            ...
            return (
              <div key={agent} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                ...
                <Badge variant="outline" className={PERMISSION_LEVEL_COLORS[level]}>
...
const CustomPermissionsEditor: React.FC<{
  ...
  const handleAgentLevelChange = useCallback((agent: AgentName, level: PermissionLevel) => {
          ...
          {Object.entries(AgentName).map(([, agentValue]) => {
            ...
            return (
              <div key={agentValue} className="space-y-2">
                ...
                <Select
                  value={currentLevel}
                  onValueChange={(level) => handleAgentLevelChange(agentValue, level as PermissionLevel)}
                  disabled={disabled}
                >
...
export const BulkPermissionDialog: React.FC<BulkPermissionDialogProps> = ({
  ...
  const handleOperationTypeChange = useCallback((type: BulkOperation['type']) => {
    setOperation(prev => {
      ...
      if (type === 'grant_all') {
        newOp.permissions = Object.values(AgentName).reduce((acc, agent) => {
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  <div>
                    <div className="text-2xl font-bold text-green-600">{userSummary.activeUsers}</div>
                    <div className="text-xs text-muted-foreground">Ativos</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-gray-600">{userSummary.inactiveUsers}</div>
              ...
              <CardContent className="space-y-4">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="grant_all">Conceder Todas as Permissões</SelectItem>
                    <SelectItem value="revoke_all">Revogar Todas as Permissões</SelectItem>
                    <SelectItem value="custom">Permissões Personalizadas</SelectItem>
                  </SelectContent>
                </Select>
            ...
            {operation.type === 'custom' && operation.permissions && (
              <CustomPermissionsEditor
                permissions={operation.permissions}
                onChange={(permissions) => setOperation(prev => ({ ...prev, permissions }))}
                disabled={progress.isRunning}
              />
            )}
            ...
            {(operation.type === 'grant_all' || operation.type === 'revoke_all') && (
