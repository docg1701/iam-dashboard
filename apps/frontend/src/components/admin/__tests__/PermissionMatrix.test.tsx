...
describe('PermissionMatrix Component', () => {
  beforeEach(async () => {
    await act(async () => {
      ...
      vi.mocked(global.fetch).mockImplementation((url, options) => {
        ...
        if (urlStr.includes('/permissions/user/') && options?.method === 'GET') {
        ...
        if (urlStr.includes('/permissions/') && (options?.method === 'PUT' || options?.method === 'POST')) {
          return Promise.resolve({
            ok: true,
            status: options?.method === 'POST' ? 201 : 200,
            json: () => Promise.resolve({ success: true, message: 'Permission updated successfully' }),
            headers: new Headers({ 'content-type': 'application/json' })
          } as Response)
        }
  ...
  describe('Basic Rendering', () => {
    it('should render permission matrix table with users and agents', async () => {
      ...
      await waitFor(() => {
        ...
        const tableOrGrid = screen.queryByRole('table') ||
    ...
    it('should render permission badges for each user-agent combination', async () => {
      ...
      await waitFor(() => {
        ...
        const permissionElements = screen.queryAllByRole('button') ||
    ...
    it('should show user status indicators', async () => {
      ...
      await waitFor(() => {
        ...
        const statusElements = screen.queryAllByText(/ativo|inativo|active|inactive/i)
  ...
  describe('Loading States', () => {
    it('should show loading skeleton while fetching data', async () => {
      ...
      await waitFor(() => {
        const loadingElements = screen.queryAllByTestId(/skeleton|loading/) ||
  ...
  describe('Permission Editing', () => {
    it('should allow clicking permission elements to edit', async () => {
      ...
      await waitFor(() => {
        ...
        const editableElements = screen.queryAllByRole('button', { name: /permissão|permission/i })
        expect(editableElements.length).toBeGreaterThan(0)
      })

      // Click on first editable permission element
      const editableElements = screen.getAllByRole('button', { name: /permissão|permission/i })
      if (editableElements.length > 0) {
        await user.click(editableElements[0])

        // Should open some kind of editing interface
        await waitFor(() => {
          const editingInterface = screen.queryByRole('combobox') ||
  ...
  describe('Bulk Selection', () => {
    it('should allow selecting multiple users with checkboxes', async () => {
      ...
      if (checkboxes.length >= 2) {
        ...
        await waitFor(() => {
          const bulkIndicator = screen.queryByText(/selecionados|selected/) ||
  ...
  describe('Responsive Design', () => {
    it('should display horizontally scrollable table on mobile', async () => {
      ...
      await waitFor(() => {
        ...
        const container = screen.queryByTestId('permission-matrix-container') ||
  ...
  describe('Accessibility', () => {
    it('should have proper ARIA labels and roles', async () => {
      ...
      await waitFor(() => {
        ...
        const structuredElement = screen.queryByRole('table') ||
                                 screen.queryByRole('grid')
