...
describe("BulkPermissionDialog", () => {
  ...
  describe("Dialog Rendering", () => {
    ...
    it("should render with zero users when no users selected", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            /nenhum usuário selecionado|0 usuários|não há usuários/i,
          ) || screen.getByText(/selecione usuários|usuários para aplicar/i),
        ).toBeInTheDocument();
      });
    });
    ...
    it("should render dialog header with selected user count", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/operações em lote.*3.*usuários selecionados/i),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Change Reason Requirement", () => {
    it("should require change reason for operations", async () => {
      ...
      await waitFor(() => {
        const reasonField =
          screen.queryByPlaceholderText(/motivo|razão|descreva/i) ||
    ...
    it("should show apply/action buttons", async () => {
      ...
      await waitFor(() => {
        const actionButtons = screen.queryByRole("button", {
          name: /aplicar|apply|salvar|save/i,
        });
