...
const mockAuditLogs = [
  ...
  },
  {
    ...
    change_reason:
      "Mudança de função - não requer mais processamento de documentos",
    changed_by_user_id: "admin-123",
...
describe("PermissionAuditLog Component", () => {
  ...
  describe("Embedded Mode Rendering", () => {
    it("should render audit log with entries in embedded mode", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.queryByText("Carregando histórico..."),
    ...
    it("should display different action types with appropriate badges", async () => {
      ...
      await waitFor(() => {
        ...
        const actionElements = screen.queryAllByText(
          /concedido|revogado|template|grant|revoke/i,
        );
    ...
    it("should show agent and permission change information", async () => {
      ...
      await waitFor(() => {
        ...
        const agentElements =
          screen.queryByText(/gestão|clientes|processamento|relatórios/i) ||
    ...
    it("should display timestamps in formatted time", async () => {
      ...
      await waitFor(() => {
        ...
        const dateElements =
          screen.queryAllByText(/\d{2}\/\d{2}\/\d{4}|\d{4}-\d{2}-\d{2}/) ||
    ...
    it("should not show filters in embedded mode", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.queryByPlaceholderText(/buscar por usuário/i),
        ).not.toBeInTheDocument();
        expect(
    ...
    it("should show empty state when no audit logs exist", async () => {
      ...
      await waitFor(() => {
        const emptyMessage = screen.queryByText(
          /nenhum registro|não há registros|empty|no records/i,
        );
  ...
  describe("Table Structure", () => {
    it("should have proper table structure with headers", async () => {
      ...
      await waitFor(() => {
        ...
        if (table) {
          ...
          const structuredContent = screen.queryByText(
            /data|usuário|agente|ação/i,
          );
    ...
    it("should show user column by default", async () => {
      await act(async () => {
        renderWithProviders(
          <PermissionAuditLog embedded showUserColumn={true} />,
        );
      });
      ...
      await waitFor(() => {
        ...
        const userContent =
          screen.queryByText(/usuário|user/) || screen.queryByText("user-123");
    ...
    it("should hide user column when showUserColumn is false", async () => {
      await act(async () => {
        renderWithProviders(
          <PermissionAuditLog embedded showUserColumn={false} />,
        );
      });
      ...
      await waitFor(() => {
        ...
        const content = screen.queryByText(
          /audit|histórico|registro|data|agente/i,
        );
