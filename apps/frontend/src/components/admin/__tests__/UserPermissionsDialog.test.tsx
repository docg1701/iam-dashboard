...
describe("UserPermissionsDialog", () => {
  ...
  const renderComponent = async (
    user: typeof mockTestUser | null = mockTestUser,
    open = true,
  ) => {
    ...
    it("should render dialog header with user name", async () => {
      ...
      await waitFor(
        async () => {
          await act(async () => {
            expect(
              screen.getByText(`Permissões de ${mockTestUser.full_name}`),
            ).toBeInTheDocument();
            expect(
              screen.getByText(/gerencie as permissões de acesso aos agentes/i),
            ).toBeInTheDocument();
          });
        },
        { timeout: 3000 },
      ...
      await waitFor(
        async () => {
          await act(async () => {
            ...
            expect(
              screen.getByText(mockTestUser.full_name!),
            ).toBeInTheDocument();
    ...
    it("should display permissions form when loaded successfully", async () => {
      ...
      await waitFor(async () => {
        await act(async () => {
          expect(screen.getByText(/motivo da alteração/i)).toBeInTheDocument();
          expect(
            screen.getByText(/permissões por agente/i),
          ).toBeInTheDocument();
          expect(
            screen.getByPlaceholderText(/descreva o motivo desta alteração/i),
          ).toBeInTheDocument();
        });
      });
    });
    ...
    it("should display all agent permission cards", async () => {
      ...
      await waitFor(async () => {
        await act(async () => {
          ...
          expect(
            screen.getByText(/processamento de pdfs/i),
          ).toBeInTheDocument();
          expect(
            screen.getByText(/relatórios e análises/i),
          ).toBeInTheDocument();
  ...
  describe("Permission Level Selection", () => {
    ...
    it("should update individual checkboxes when permission level changes", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/você tem alterações não salvas/i),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Change Reason Requirement", () => {
    ...
    it("should require change reason to enable save button", async () => {
      ...
      const reasonTextarea = screen.getByPlaceholderText(
        /descreva o motivo desta alteração/i,
      );
      await user.type(
        reasonTextarea,
        "Usuário precisa de permissão de exclusão para casos especiais",
      );
  ...
  describe("Permission History", () => {
    ...
    it("should show/hide permission history when toggle clicked", async () => {
      ...
      expect(
        screen.queryByText(/histórico de alterações/i),
  ...
  describe("Form Submission", () => {
    ...
    it("should save permissions when save button is clicked", async () => {
      ...
      await waitFor(() => {
        expect(mockOnPermissionsChanged).toHaveBeenCalledWith(
          mockTestUser.user_id,
        );
    ...
    it("should have proper dialog structure", async () => {
      ...
      expect(
        screen.getByRole("heading", {
          name: new RegExp(`permissões de ${mockTestUser.full_name}`, "i"),
        }),
      ).toBeInTheDocument();
    });
  ...
  describe("Responsive Behavior", () => {
    ...
    it("should layout agent cards in responsive grid", async () => {
      ...
      const agentGrid = screen
        .getByText(/gestão de clientes/i)
        .closest(".grid");
