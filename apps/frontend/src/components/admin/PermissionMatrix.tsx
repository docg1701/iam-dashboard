...
const PermissionCell: React.FC<PermissionCellProps & { inCard?: boolean }> = ({
  ...
  const badgeContent = (
    <Badge
      ...
      aria-label={
        isEditable
          ? `Alterar permissão ${PERMISSION_LEVEL_NAMES[level]} para ${userName} - ${AGENT_DISPLAY_NAMES[agent]}`
          : `Permissão atual: ${PERMISSION_LEVEL_NAMES[level]} para ${userName} - ${AGENT_DISPLAY_NAMES[agent]}`
      }
  ...
  const selectContent = (
    ...
    >
      ...
      <SelectContent
        ...
        data-testid={`permission-select-content-${userId}-${agent}`}
      >
  ...
  if (inCard) {
    return (
      <div className="text-center" data-testid={`permission-cell-${agent}`}>
        ...
        {isEditing && (
          <div data-testid="permission-updating-indicator" className="sr-only">
...
export const PermissionMatrix: React.FC<PermissionMatrixProps> = ({
  ...
  const { data: userPermissions, isLoading: permissionsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const permissionPromises = userIds.map(async (userId) => {
        try {
          const userMatrix =
            await PermissionAPI.User.getUserPermissions(userId);
    ...
    mutationFn: async ({
      ...
      try {
        ...
        const response = await fetch(
          `/api/v1/permissions/user/${userId}/agent/${agent}`,
          {
            method: "PUT",
            headers: {
  ...
  const handlePermissionChange = useCallback(
    (userId: string, agent: AgentName, permissions: PermissionActions) => {
    ...
    (action: string) => {
      if (selectedUsers.size === 0) {
        addToast({
          ...
          description:
            "Selecione pelo menos um usuário para executar ações em lote.",
          variant: "error",
        });
        return;
      }
  ...
  const MobileView = () => (
    <div
      ...
      data-testid="permission-matrix-cards"
    >
            ...
            >
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent data-testid="bulk-actions-content">
                  ...
                  <DropdownMenuItem
                    onClick={() => handleBulkAction("grant_all")}
                    data-testid="bulk-action-grant"
                  >
                    Conceder Todas Permissões
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => handleBulkAction("revoke_all")}
                    data-testid="bulk-action-revoke"
                  >
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
              ...
              >
                ...
                <SelectContent data-testid="filter-role-options">
                  <SelectItem value="all" data-testid="filter-role-all">
                    Todos os Cargos
                  </SelectItem>
                  <SelectItem
                    value="sysadmin"
                    data-testid="filter-role-sysadmin"
                  >
                    Administrador Sistema
                  </SelectItem>
                  <SelectItem value="admin" data-testid="filter-role-admin">
                    Admin
                  </SelectItem>
                  <SelectItem value="user" data-testid="filter-role-user">
                    Usuário
                  </SelectItem>
                </SelectContent>
              </Select>
              ...
              >
                ...
                <SelectContent data-testid="filter-status-options">
                  ...
                  <SelectItem value="active" data-testid="filter-status-active">
                    Ativo
                  </SelectItem>
                  <SelectItem
                    value="inactive"
                    data-testid="filter-status-inactive"
                  >
                    Inativo
                  </SelectItem>
                </SelectContent>
                  ...
                  <p className="text-2xl font-bold">
                    {
                      filteredUsers.filter((u) => {
                        ...
                        return (
                          perms &&
                          Object.values(perms.permissions).some((p) =>
                  ...
                  <p className="text-2xl font-bold">
                    {
                      filteredUsers.filter((u) => {
                        ...
                        return (
                          !perms ||
                          Object.values(perms.permissions).every((p) =>
        ...
        <Card className="hidden md:block">
          <CardContent className="p-0">
            <div
              ...
              data-testid="permission-matrix-container"
            >
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead
                      ...
                      role="columnheader"
                      data-testid="column-header-user"
                    >
                      Usuário
                    </TableHead>
                    <TableHead
                      className="w-20"
                      role="columnheader"
                      data-testid="column-header-role"
                    >
                      Cargo
                    </TableHead>
                    <TableHead
                    ...
                    {Object.entries(AGENT_DISPLAY_NAMES).map(
                      ([agentKey, agentName]) => (
                        <TableHead
                          ...
                          role="columnheader"
                          data-testid={`column-header-${agentKey}`}
                        >
                          {agentName}
                        </TableHead>
                      ),
                      ...
                      >
                        <TableCell>
                          <input
                            ...
                            checked={isSelected}
                            onChange={(e) =>
                              handleUserSelect(user.user_id, e.target.checked)
                            }
                            className="rounded border-gray-300"
                        ...
                        {Object.entries(AgentName).map(([, agentValue]) => {
                          const permissions =
                            userPerms?.permissions[agentValue];
                          return (
                            <UpdatePermissionGuard
                              key={agentValue}
                              agent={AgentName.CLIENT_MANAGEMENT}
