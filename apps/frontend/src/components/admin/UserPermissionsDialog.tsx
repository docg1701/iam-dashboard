...
const AgentPermissionEditor: React.FC<{
  ...
  const handleIndividualPermissionChange = useCallback(
    (operation: keyof PermissionActions, value: boolean) => {
...
export const UserPermissionsDialog: React.FC<UserPermissionsDialogProps> = ({
  ...
  const handlePermissionChange = useCallback(
    (agent: AgentName, permissions: PermissionActions) => {
      setFormData((prev) => ({
        ...prev,
        permissions: {
          ...prev.permissions,
        ...
        >
            ...
            <CardContent className="pt-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Badge
                    variant={user.role === "sysadmin" ? "default" : "outline"}
                  >
                    {user.role}
                  </Badge>
                </div>
          ...
          {!isLoading && !permissionsError && (
            <div className="space-y-6">
                  ...
                  {Object.entries(AgentName).map(([, agentValue]) => (
                    ...
                    >
                      <AgentPermissionEditor
                        agent={agentValue}
                        permissions={
                          formData.permissions[agentValue] ||
                          DEFAULT_PERMISSIONS
                        }
              ...
              {/* Permission History */}
              {showHistory && <PermissionHistory userId={user.user_id} />}
          ...
          <DialogFooter>
            ...
            <UpdatePermissionGuard agent={AgentName.CLIENT_MANAGEMENT}>
              <Button
                onClick={handleSave}
                disabled={
                  !hasChanges || isSaving || !formData.changeReason.trim()
                }
