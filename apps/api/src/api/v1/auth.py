...
async def get_user_permissions(
    ...
    user_permissions: dict[str, PermissionSet] = {}
    for perm in permissions:
        user_permissions[perm.agent_name.value] = PermissionSet(
...
@router.post(
    "/login", response_model=LoginResponse, summary="User login with optional 2FA"
)
async def login(
    login_request: LoginRequest, session: AsyncSession = Depends(get_async_session)
) -> LoginResponse:
    """
    ...
    try:
        ...
        if not auth_service.verify_password(login_request.password, user.password_hash):
            ...
            if user.failed_login_attempts >= 5:
                user.locked_until = datetime.now(UTC).replace(
        ...
        assert user.id is not None  # user is validated, id cannot be None
...
@router.post(
    "/logout", response_model=MessageResponse, summary="Logout current session"
)
async def logout(
...
@router.post(
    "/logout-all", response_model=MessageResponse, summary="Logout all sessions"
)
...
async def setup_2fa(
    ...
    try:
        ...
        backup_codes = [
            "".join([str(secrets.randbelow(10)) for _ in range(8)]) for _ in range(8)
        ]
        ...
        return Setup2FAResponse(
            secret=secret, qr_code_url=qr_url, backup_codes=backup_codes
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="2FA setup failed"
        ) from e


@router.post(
    "/enable-2fa", response_model=MessageResponse, summary="Enable 2FA for user"
)
async def enable_2fa(
    ...
    try:
        ...
        secret: str | None = auth_service.redis_client.get(f"totp_setup:{user_id}")
        if not secret:
            raise HTTPException(
...
@router.delete(
    "/disable-2fa", response_model=MessageResponse, summary="Disable 2FA for user"
)
...
async def get_current_user(
    ...
    try:
        ...
        # Get user from database
        statement = select(User).where(User.id == uuid.UUID(user_id), User.is_active)
        result = await session.execute(statement)
        user = result.scalar_one_or_none()
        if not user:
            raise HTTPException(
        ...
        logger.info(
            "User info retrieved successfully", user_id=str(user.id), email=user.email
        )
