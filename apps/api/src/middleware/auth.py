...
class UserDict(TypedDict):
    """User information dictionary structure."""
...
class SessionInfo(TypedDict):
    """User session information structure."""
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: FastAPI, exclude_paths: list[str] | None = None) -> None:
        super().__init__(app)
    ...
    async def dispatch(
        ...
        try:
            ...
            request.state.auth_context.user_role = (
...
async def get_current_user(
    ...
    try:
        ...
        logger.error("Authentication dependency failed", error=str(e.detail))
        raise e


def require_role(
    required_role: UserRole,
) -> Callable[..., Awaitable[UserDict]]:
    """
    ...
    async def role_dependency(
        ...
        if not user_role:
            logger.error(
                "Missing user role in token", user_id=current_user.get("user_id")
            )
            raise HTTPException(
        ...
        role_hierarchy = {UserRole.USER: 1, UserRole.ADMIN: 2, UserRole.SYSADMIN: 3}
...
async def require_sysadmin(
    current_user: Annotated[UserDict, Depends(require_role(UserRole.SYSADMIN))],
) -> UserDict:
...
async def require_admin(
    current_user: Annotated[UserDict, Depends(require_role(UserRole.ADMIN))],
) -> UserDict:
...
async def require_user(
    current_user: Annotated[UserDict, Depends(require_role(UserRole.USER))],
) -> UserDict:
...
class PermissionService:
    ...
    async def check_agent_permission(
        self, user_id: str, agent_name: AgentName, operation: str
    ) -> bool:
        """
    ...
    async def get_user_permissions(
        ...
        logger.debug("User permissions lookup requested", user_id=user_id)
...
def require_agent_permission(
    ...
    async def permission_dependency(
        ...
        has_permission = await permission_service.check_agent_permission(
            user_id=user_id, agent_name=agent_name, operation=operation
        )
...
async def get_user_session_info(user_id: str) -> SessionInfo:
    """
    ...
    try:
        ...
        logger.error("Failed to get user session info", user_id=user_id, error=str(e))
        return {
            "user_id": user_id,
            "active_sessions": 0,
            "max_sessions": 5,
...
async def invalidate_user_sessions(
    user_id: str, keep_current_token: str | None = None
) -> int:
    """
    ...
    try:
        ...
        for token_bytes in sessions:
            ...
            if token_str != keep_current_token:
                # Blacklist the token
                auth_service.blacklist_token(token_str)
                # Remove from session set
                auth_service.redis_client.srem(session_key, token_str)
                invalidated_count += 1
...
def get_current_user_role(request: Request) -> UserRole | None:
    """
