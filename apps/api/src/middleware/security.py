...
def get_rate_limit_key(request: Request) -> str:
    ...
    if user_role:
        ...
        if user_id:
            ...
            role_prefix = user_role.value if isinstance(user_role, UserRole) else str(user_role)
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self.settings.ENABLE_SECURITY_HEADERS:
            ...
            response.headers["Permissions-Policy"] = "geolocation=(), microphone=(), camera=(), payment=(), usb=(), magnetometer=(), gyroscope=(), speaker=(), vibrate=()"
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            response.headers["X-RateLimit-Reset"] = str(int((datetime.now() + timedelta(seconds=limit_window)).timestamp()))
...
def get_rate_limit_by_role(role: Optional[UserRole]) -> str:
    """
