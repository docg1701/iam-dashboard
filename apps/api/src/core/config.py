...
class Settings(BaseSettings):
    ...
    ACCESS_TOKEN_EXPIRE_MINUTES: int = Field(
        default=60, description="Access token expiration time (1 hour)"
    )
    REFRESH_TOKEN_EXPIRE_DAYS: int = Field(
        default=30, description="Refresh token expiration time (30 days)"
    )

    # 2FA Settings
    TOTP_SECRET_LENGTH: int = Field(
        default=32, description="TOTP secret length in bytes"
    )
    TOTP_TOKEN_VALIDITY: int = Field(
        default=30, description="TOTP token validity window in seconds"
    )
    TOTP_ISSUER_NAME: str = Field(
        default="IAM Dashboard", description="TOTP issuer name"
    )
    ...
    MFA_SETUP_TOKEN_EXPIRE_MINUTES: int = Field(
        default=10, description="MFA setup token expiration"
    )

    # Rate Limiting Configuration
    USER_RATE_LIMIT_PER_MINUTE: int = Field(
        default=100, description="Rate limit for regular users (requests/minute)"
    )
    ADMIN_RATE_LIMIT_PER_MINUTE: int = Field(
        default=500, description="Rate limit for admin users (requests/minute)"
    )
    SYSADMIN_RATE_LIMIT_PER_MINUTE: int = Field(
        default=1000, description="Rate limit for sysadmin users (requests/minute)"
    )
    GLOBAL_RATE_LIMIT_PER_MINUTE: int = Field(
        default=5000, description="Global rate limit (requests/minute)"
    )
    RATE_LIMIT_STORAGE_URL: str | None = Field(
        default=None, description="Redis URL for rate limiting storage"
    )
    ...
    SESSION_REFRESH_THRESHOLD_MINUTES: int = Field(
        default=60, description="Refresh session if expires within X minutes"
    )
    ...
    CSP_POLICY: str = Field(
        default="default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' https://api.*.com.br wss://api.*.com.br",
        description="Content Security Policy",
    )
    REFERRER_POLICY: str = Field(
        default="strict-origin-when-cross-origin", description="Referrer policy"
    )
    ...
    COOKIE_DOMAIN: str | None = Field(
        default=None, description="Cookie domain for production"
    )
    ...
    PERMISSION_CACHE_TTL: int = Field(
        default=300, description="Permission cache TTL in seconds"
    )
    PERMISSION_CHECK_TIMEOUT: int = Field(
        default=10, description="Permission check timeout in milliseconds"
    )
