...
class AuthService:
    ...
    def create_refresh_token(
        ...
        self.redis_client.setex(
            f"refresh_token:{jti}",
            int(expire.timestamp() - datetime.now(timezone.utc).timestamp()),
            str(user_id)
        )
    ...
    def _store_user_session(self, user_id: str, token: str, expire: datetime) -> None:
        ...
        if len(current_sessions) >= 5:
            ...
            oldest_session = list(current_sessions)[0]
    ...
    def logout_all_sessions(self, user_id: str) -> None:
        ...
        logger.info("User logged out from all sessions", user_id=user_id, session_count=len(sessions))
    ...
    async def _get_user_role_from_db(self, user_id: str) -> str:
        ...
        try:
            ...
            async with session_maker() as session:
                ...
                logger.debug("User role retrieved successfully", user_id=user_id, role=user.role.value)
                return user.role.value
                
        except ValueError as e:
            # Invalid UUID format
            logger.error("Invalid user ID format during role lookup", user_id=user_id, error=str(e))
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Invalid user ID"
            )
        except Exception as e:
            logger.error("Database error during user role lookup", user_id=user_id, error=str(e))
            raise HTTPException(
