...
from ..middleware.auth import get_current_user, require_sysadmin
from ..middleware.security import limiter, get_rate_limit_by_role
...
def get_user_rate_limit_key(request: Request) -> str:
    ...
    return f"user:example:{request.client.host}"
...
async def role_based_rate_limit_test(
    ...
    """
    user_role = UserRole(current_user.get("user_role")) if current_user.get("user_role") else None
...
async def secure_login_example(request: Request):
    """
...
async def secure_logout_example(request: Request):
    """
...
async def sysadmin_only_endpoint(
    current_user: Dict[str, Any] = Depends(require_sysadmin)
):
    """
...
async def security_headers_test():
    """
