...
from factories import UserFactory, ClientFactory, UserAgentPermissionFactory, AuditLogFactory
...
class DatabaseSeeder:
    ...
    async def seed_initial_users(self) -> dict:
        ...
        sysadmin = UserFactory.create_sysadmin(
            email="sysadmin@iam-dashboard.local",
        ...
        admin = UserFactory.create_admin(
            email="admin@iam-dashboard.local",
            password="AdminUser@2025"
        )
    ...
    async def seed_sample_clients(self, users: dict) -> list[Client]:
        ...
        user1_clients = ClientFactory.create_clients_for_user(
            user_id=user1_id,
            count=5
        )
        
        # User2 creates some clients
        user2_clients = ClientFactory.create_clients_for_user(
            user_id=user2_id,
            count=3
        )
    ...
    async def seed_permissions(self, users: dict) -> list[UserAgentPermission]:
        ...
        admin_permissions = UserAgentPermissionFactory.create_agent_permissions_for_user(
            user_id=admin_id,
    ...
    async def seed_audit_logs(self, users: dict, clients: list[Client]) -> list[AuditLog]:
    ...
    async def run_full_seed(self) -> dict:
        ...
        try:
            ...
            # Seed permissions
            permissions = await self.seed_permissions(users)
            
            # Seed audit logs
            audit_logs = await self.seed_audit_logs(users, clients)
