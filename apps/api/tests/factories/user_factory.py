...
class UserFactory(BaseFactory):
    ...
    @classmethod
    def create_sysadmin(self, email: str | None = None, **kwargs) -> User:
        """Create a sysadmin user."""
        return self.create_user(
            email=email or "sysadmin@example.com", role=UserRole.SYSADMIN, **kwargs
        )

    @classmethod
    def create_admin(self, email: str | None = None, **kwargs) -> User:
        """Create an admin user."""
        return self.create_user(
            email=email or "admin@example.com", role=UserRole.ADMIN, **kwargs
        )

    @classmethod
    def create_regular_user(self, email: str | None = None, **kwargs) -> User:
        """Create a regular user."""
        return self.create_user(
            email=email or self.generate_email(), role=UserRole.USER, **kwargs
        )

    @classmethod
    def create_user_with_2fa(self, email: str | None = None, **kwargs) -> User:
        """Create a user with 2FA enabled."""
        totp_secret = secrets.token_hex(16)
        return self.create_user(email=email, totp_secret=totp_secret, **kwargs)

    @classmethod
    def create_locked_user(
        self, email: str | None = None, lock_duration_hours: int = 24, **kwargs
    ) -> User:
        """Create a locked user account."""
        locked_until = datetime.now(UTC) + timedelta(hours=lock_duration_hours)
        return self.create_user(
            email=email,
    ...
    @classmethod
    def create_inactive_user(self, email: str | None = None, **kwargs) -> User:
        """Create an inactive user account."""
        return self.create_user(email=email, is_active=False, **kwargs)

    @classmethod
    def create_user_with_login_history(
        self, email: str | None = None, days_since_login: int = 1, **kwargs
    ) -> User:
        """Create a user with recent login history."""
        last_login = datetime.now(UTC) - timedelta(days=days_since_login)
        return self.create_user(email=email, last_login_at=last_login, **kwargs)

    @classmethod
    def create_multiple_users(
        self, count: int, role_distribution: dict | None = None, **kwargs
    ) -> list[User]:
        """
        ...
        for i in range(count):
            ...
            if i < count * role_distribution.get(UserRole.SYSADMIN, 0):
                ...
                role_distribution.get(UserRole.SYSADMIN, 0)
            ...
            user = self.create_user(
                role=role, email=self.generate_email(domain="testdomain.com"), **kwargs
            )
            users.append(user)
