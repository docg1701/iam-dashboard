...
class TestUserAgentPermissionModel:
    ...
    def test_expired_permission_creation(self):
        ...
        assert permission.expires_at < datetime.now(UTC)

        # Verify it's properly expired
        expected_expiry = datetime.now(UTC) - timedelta(days=days_expired)
    ...
    def test_expiring_permission_creation(self):
        ...
        # Verify expiry date
        expected_expiry = datetime.now(UTC) + timedelta(days=days_until_expiry)
        time_diff = abs((permission.expires_at - expected_expiry).total_seconds())
        assert time_diff < 60  # Less than 1 minute difference
    ...
    def test_agent_permissions_for_user(self):
        ...
        permissions = UserAgentPermissionFactory.create_agent_permissions_for_user(
            user_id=user.id, granted_by=admin.id, permission_type="full"
        )
    ...
    def test_permission_matrix(self):
        ...
        permissions = UserAgentPermissionFactory.create_permission_matrix(
            users=user_ids, admin_id=admin.id
        )
        ...
        for permission in permissions:
            if (
                ...
                and permission.can_read
                and permission.can_update
                and not permission.can_delete
            ):
                permission_types.add("read_write")
            elif (
                not permission.can_create
                and permission.can_read
                and not permission.can_update
                and not permission.can_delete
            ):
    ...
    def test_permission_id_uniqueness(self):
        ...
        permissions = [
            UserAgentPermissionFactory.create_permission() for _ in range(10)
        ]
...
class TestUserAgentPermissionProperties:
    ...
    def test_has_any_permission_property(self):
        ...
        create_permission = UserAgentPermissionFactory.create_permission(
            can_create=True
        )
        ...
        update_permission = UserAgentPermissionFactory.create_permission(
            can_update=True
        )
        assert update_permission.has_any_permission is True

        # Has delete permission
        delete_permission = UserAgentPermissionFactory.create_permission(
            can_delete=True
        )
    ...
    def test_is_valid_property(self):
        ...
        inactive_expired = UserAgentPermissionFactory.create_expired_permission(
            is_active=False
        )
...
class TestUserAgentPermissionRepr:
    ...
    def test_permission_repr(self):
        ...
        assert "permissions=CR" in repr_str  # Should show Create and Read permissions
    ...
    def test_permission_repr_full_access(self):
        ...
        repr_str = repr(permission)
        assert "permissions=CRUD" in repr_str  # Should show all four permissions
    ...
    def test_permission_repr_no_permissions(self):
        ...
        repr_str = repr(permission)
        assert "UserAgentPermission(" in repr_str
        # Should contain "none" for no permissions
        assert "permissions=none" in repr_str
...
class TestAgentNameEnum:
    ...
    def test_agent_name_comparison(self):
        ...
        assert (
            AgentName.CLIENT_MANAGEMENT == "client_management"
        )  # str enum equals its string value
...
class TestUserAgentPermissionValidation:
    ...
    def test_permission_requires_user_id(self):
        ...
        with pytest.raises((ValidationError, TypeError)):
            UserAgentPermission(
                agent_name=AgentName.CLIENT_MANAGEMENT, granted_by=uuid.uuid4()
            )

    def test_permission_requires_agent_name(self):
        """Test that agent_name is required."""
        with pytest.raises((ValidationError, TypeError)):
            UserAgentPermission(user_id=uuid.uuid4(), granted_by=uuid.uuid4())

    def test_permission_requires_granted_by(self):
        """Test that granted_by is required."""
        with pytest.raises((ValidationError, TypeError)):
            UserAgentPermission(
                user_id=uuid.uuid4(), agent_name=AgentName.CLIENT_MANAGEMENT
            )
