...
class TestAuditLogModel:
    ...
    def test_security_audit_logs(self):
        ...
        failed_login = next((log for log in security_logs 
                           if log.additional_data and 
                           log.additional_data.get("success") is False), None)
        ...
        suspicious_login = next((log for log in security_logs 
                               if log.additional_data and 
                               log.additional_data.get("suspicious") is True), None)
        assert suspicious_login is not None
        
        # Should include permission escalation
        perm_change = next((log for log in security_logs 
                          if log.action == AuditAction.PERMISSION_CHANGE), None)
...
class TestAuditActionEnum:
    ...
    def test_audit_action_comparison(self):
        ...
        assert AuditAction.LOGIN != "login"  # Different types
