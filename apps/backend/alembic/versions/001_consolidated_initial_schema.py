...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE agentname AS ENUM ('client_management', 'pdf_processing', 'reports_analysis', 'audio_recording')")
    ...
    op.create_table(
        ...
        sa.Column('permissions', JSONB(), nullable=False, server_default='{"create": false, "read": false, "update": false, "delete": false}'),
    ...
    op.create_index('ix_user_agent_permissions_user_agent', 'user_agent_permissions', ['user_id', 'agent_name'], unique=True)
    op.create_index('ix_user_agent_permissions_created_at', 'user_agent_permissions', ['created_at'])
    ...
    op.create_index('ix_permission_templates_name', 'permission_templates', ['template_name'], unique=True)
    ...
    op.create_table(
        ...
        sa.CheckConstraint("action IN ('CREATE', 'UPDATE', 'DELETE', 'BULK_ASSIGN')", name='audit_action_valid')
    )
    ...
    op.create_index('ix_permission_audit_user_agent', 'permission_audit_log', ['user_id', 'agent_name'])
...
def downgrade() -> None:
    ...
    op.execute("DROP TRIGGER IF EXISTS update_user_agent_permissions_updated_at ON user_agent_permissions;")
    op.execute("DROP TRIGGER IF EXISTS update_permission_templates_updated_at ON permission_templates;")
