...
class SecurityMonitoringService:
    ...
    def _initialize_threat_patterns(self) -> dict[str, ThreatPattern]:
        ...
        patterns = {
            ...
            "session_hijacking": ThreatPattern(
                ...
                event_types=[SecurityEventType.SESSION_HIJACKING, SecurityEventType.SESSION_ANOMALY],
                threshold_count=1,
            ...
            "privilege_escalation": ThreatPattern(
                ...
                event_types=[SecurityEventType.ACCESS_DENIED, SecurityEventType.PERMISSION_ESCALATION],
                threshold_count=3,
    ...
    def _update_stats(self, event: SecurityEvent) -> None:
        ...
        try:
            ...
            # Update counters
            self.redis_client.hincrby(stats_key, f"total_events", 1)
            ...
            if event.risk_score > 0:
                self.redis_client.hincrby(stats_key, f"high_risk_events", 1)
    ...
    def _trigger_alert(self, pattern: ThreatPattern, triggering_event: SecurityEvent, event_count: int) -> None:
    ...
    def _store_alert(self, alert: SecurityAlert) -> None:
        ...
        try:
            ...
            cutoff_time = (datetime.utcnow() - timedelta(days=self.alert_retention_days)).timestamp()
    ...
    def get_security_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            stats["events_last_24h"] = stats.get(f"daily_{today_str}", 0) + stats.get(f"daily_{yesterday_str}", 0)
...
def log_login_attempt(user_id: str, ip_address: str, success: bool, details: dict[str, Any] | None = None) -> None:
    """Log a login attempt."""
...
def log_suspicious_activity(user_id: str, ip_address: str, activity: str, details: dict[str, Any]) -> None:
...
def log_data_access(user_id: str, resource: str, action: str, ip_address: str | None = None) -> None:
