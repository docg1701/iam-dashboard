...
class AdvancedRateLimiter:
    ...
    async def _is_locked_out(self, key_type: str, identifier: str, scenario: str) -> tuple[bool, int]:
    ...
    async def _apply_lockout(self, key_type: str, identifier: str, scenario: str, duration: int) -> None:
        ...
        for key_type, identifier in identifiers.items():
            if key_type in ["ip", "email", "user"]:
                result = await self.check_rate_limit(
                    identifier, scenario, key_type, client_info  # type: ignore
                )
...
async def check_login_rate_limit(ip_address: str, email: str, client_info: dict[str, Any] | None = None) -> RateLimitResult:
    """Check rate limit for login attempts."""
    return await rate_limiter.check_multiple_identifiers(
        {"ip": ip_address, "email": email},
        "login",
        client_info
    )
