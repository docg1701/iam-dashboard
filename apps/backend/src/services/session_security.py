...
class SessionSecurityService:
    ...
    def calculate_fingerprint_similarity(
        self, fingerprint1: SessionFingerprint, fingerprint2: SessionFingerprint
    ) -> float:
        """
        ...
        if fingerprint1.user_agent == fingerprint2.user_agent:
            ...
            self._calculate_string_similarity(fingerprint1.user_agent, fingerprint2.user_agent)
            > 0.8
        ):
    ...
    async def create_session(
        self, session_id: str, user_id: UUID, request: Request
    ) -> SessionSecurity:
        """
        ...
        if self.redis_client is not None:
            try:
                ...
                self.redis_client.expire(
                    user_sessions_key, timedelta(hours=self.session_timeout_hours)
                )
        ...
        logger.info(
            f"Created secure session {session_id} for user {user_id_str} with risk score {risk_score}"
        )
    ...
    async def validate_session(
        ...
        try:
            ...
            similarity = self.calculate_fingerprint_similarity(
                session_security.fingerprint, current_fingerprint
            )
    ...
    async def get_security_alerts(self, user_id: str, limit: int = 50) -> list[SecurityAlert]:
    ...
    async def _calculate_risk_score(self, user_id: str, fingerprint: SessionFingerprint) -> float:
        ...
        try:
            ...
            recent_fingerprints = self.redis_client.lrange(
                fingerprint_key, 0, 9
            )  # Last 10 fingerprints
            ...
            if any(
                pattern.lower() in fingerprint.user_agent.lower()
    ...
    async def _store_fingerprint_history(
        self, user_id: str, fingerprint: SessionFingerprint
    ) -> None:
