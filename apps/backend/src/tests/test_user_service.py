...
class TestUserService:
    ...
    async def test_database_error_handling(
        ...
        assert "Failed to check email uniqueness" in str(
            excinfo.value.message
        ) or "Failed to create user" in str(excinfo.value.message)
    ...
    async def test_integrity_error_handling(
        ...
        orig_exception = SQLAlchemyError("email constraint violation")
        integrity_error = IntegrityError("email constraint", {}, orig_exception)
        integrity_error.orig = orig_exception
