...
class TestSeedForTesting:
    """Integration test for testing data seeding function."""
    ...
    def test_seed_for_testing(self, test_session: Session) -> None:
        ...
        with patch("src.utils.seed_data.Session") as mock_session_class:
        ...
        # Verify return data matches database
        assert len(users_created) == len(users_in_db)
        assert len(clients_created) == len(clients_in_db)
        assert len(audit_logs_created) == len(audit_logs_in_db)
