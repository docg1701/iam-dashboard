...
class TestCreateAuditLog:
    """Integration test for audit log creation function with real database."""
    ...
    async def test_create_audit_log_success(self, test_session: Session, mock_request: Mock) -> None:
        ...
        assert result.new_values == {"name": "John Doe"}
        assert result.old_values is None
        assert result.audit_id is not None  # Should have been assigned by database
    ...
    async def test_create_audit_log_with_old_values(
        ...
        """Test audit log creation with old values using real database session."""
        user_id = uuid4()
        ...
        # Verify the audit log was created with correct values in database
        assert isinstance(result, AuditLog)
        assert result.table_name == "users"
        assert result.record_id == "123"
...
class TestLogDatabaseAction:
    """Integration test for database action logging function."""
    ...
    async def test_log_database_action_success(
        ...
        """Test successful database action logging with real database session."""
        user_id = uuid4()
        ...
        assert audit_log.new_values == {"name": "Client Name"}
