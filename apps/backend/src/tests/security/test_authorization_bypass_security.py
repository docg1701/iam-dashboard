...
from src.tests.factories import ClientFactory
...
class TestDirectAPIEndpointBypass:
    ...
    def test_unauthenticated_access_to_protected_endpoints(
        self, security_test_client, test_session: Session
    ):
        ...
        for method, endpoint in protected_endpoints:
            try:
                ...
                assert "401" in str(e) or "Authorization" in str(e) or "Unauthorized" in str(e), (
                    f"Unexpected error for {method} {endpoint}: {e}"
                )
                ...
                if response.status_code == 401:
                    ...
                    assert "not found" not in detail, (
                        f"Should not leak endpoint existence: {endpoint}"
                    )

    def test_invalid_token_access_to_protected_endpoints(
        self, security_test_client, malicious_jwt_tokens: list[str]
    ):
    ...
    def test_role_based_endpoint_access_control(
        ...
        admin_token = auth_service.create_access_token(
            user_id=admin_user.user_id, user_role=admin_user.role.value, user_email=admin_user.email
        )
        ...
        for method, endpoint in admin_only_endpoints:
            ...
            if method == "GET":
                ...
                response = security_test_client.client.post(
                    endpoint, json={"dummy": "data"}, headers=user_headers
                )

            assert response.status_code == 403, (
                f"Regular user should be forbidden from {method} {endpoint}, got {response.status_code}"
            )
            ...
            assert admin_response.status_code != 403, (
                f"Admin should not be forbidden from {method} {endpoint}"
            )
...
class TestAgentPermissionBoundaryBypass:
    ...
    def test_client_management_permission_bypass_prevention(
        ...
        for method, endpoint, data in client_operations:
            ...
            assert response.status_code == 403, (
                f"Should be forbidden without client_management permission: {method} {endpoint}"
            )
    ...
    def test_mixed_agent_permission_bypass_prevention(
        ...
        assert create_response.status_code == 403, (
            "Create operation should be forbidden without create permission"
        )
    ...
    def test_agent_permission_escalation_via_api_manipulation(
        ...
        for attempt_data in escalation_attempts:
            response = security_test_client.client.post(
                "/api/v1/clients", json=attempt_data, headers=headers
            )

            # Should be forbidden regardless of manipulation attempts
            assert response.status_code == 403, (
                f"Permission escalation should be prevented: {attempt_data}"
            )
...
class TestResourceOwnershipBypass:
    ...
    def test_cross_user_resource_access_prevention(
        ...
        token_response = auth_service.create_access_token(
            user_id=user1.user_id, user_role=user1.role.value, user_email=user1.email
        )
    ...
    def test_resource_id_manipulation_prevention(
        ...
        legitimate_client = ClientFactory(
            name="Legitimate Client", created_by_user_id=regular_user.user_id
        )
        ...
        for manipulated_id in manipulated_ids:
            response = security_test_client.client.get(
                f"/api/v1/clients/{manipulated_id}", headers=headers
            )

            # Should properly handle invalid IDs without crashing or leaking data
            assert response.status_code in [400, 404, 422], (
                f"Invalid ID should be handled properly: {manipulated_id}"
            )

            # Should not return sensitive data
            if response.status_code == 200:
                data = response.json()
                assert data.get("client_id") != manipulated_id or data.get(
                    "created_by_user_id"
                ) == str(regular_user.user_id), "Should not return unauthorized data"
    ...
    def test_batch_operation_ownership_bypass_prevention(
        ...
        batch_data = {
            "client_ids": [
                str(client1.client_id),
                str(client2.client_id),
            ],  # Mix of owned and unowned
            "operation": "update",
            "data": {"name": "Batch Updated"},
        }

        response = security_test_client.client.post(
            "/api/v1/clients/batch", json=batch_data, headers=headers
        )

        # Should either:
        # 1. Reject entire batch due to unauthorized resource
        # 2. Process only authorized resources and report unauthorized ones
        assert response.status_code in [200, 403, 207], (
            f"Batch operation should handle authorization properly, got {response.status_code}"
        )
        ...
        if response.status_code == 200:
            ...
            if "processed" in result:
                assert str(client2.client_id) not in result.get("processed", []), (
                    "Unauthorized resource should not be processed"
                )
        elif response.status_code == 207:  # Partial success
            # Should report which resources were unauthorized
            result = response.json()
            assert "unauthorized" in result or "failed" in result, (
                "Should report unauthorized resources"
            )
...
class TestHTTPMethodManipulationBypass:
    ...
    def test_method_override_bypass_prevention(
        ...
        for override_header in method_override_attempts:
            ...
            response = security_test_client.client.get(
                f"/api/v1/clients/{test_client.client_id}", headers=test_headers
            )
            ...
            if response.status_code == 200:
                ...
                assert response.status_code != 405, (
                    "Method override should not change actual HTTP method processing"
                )
    ...
    def test_verb_tampering_bypass_prevention(
        ...
        verb_attempts = [
            (
                "POST",
                "/api/v1/clients",
                {"name": "Test", "cpf": "123456789", "birth_date": "1990-01-01"},
            ),
        ...
        for method, endpoint, data in verb_attempts:
            ...
            assert response.status_code in [400, 403, 404, 422], (
                f"{method} {endpoint} should require proper authorization"
            )
...
class TestParameterPollutionBypass:
    ...
    def test_query_parameter_pollution_bypass_prevention(
        ...
        pollution_attempts = [
            f"/api/v1/clients?user_id={regular_user.user_id}&user_id=admin",
    ...
    def test_json_parameter_pollution_bypass_prevention(
        ...
        for polluted_data in polluted_json_attempts:
            response = security_test_client.client.post(
                "/api/v1/clients", json=polluted_data, headers=headers
            )
            ...
            if response.status_code == 201:
                ...
                assert "role" not in created_client or created_client["role"] != "admin", (
                    "JSON pollution should not grant admin privileges"
                )
                assert "permissions" not in created_client or "admin" not in created_client.get(
                    "permissions", []
                ), "Should not grant admin permissions via pollution"
    ...
    def test_header_pollution_bypass_prevention(
        ...
        if response.status_code == 200:
            ...
            assert user_data.get("role") == regular_user.role.value, (
                "Header pollution should not change user role"
            )
            assert user_data.get("user_id") == str(regular_user.user_id), (
                "Header pollution should not change user identity"
            )
...
class TestMiddlewareBypassAttempts:
    ...
    def test_cors_bypass_attempt_prevention(
        ...
        for origin in malicious_origins:
            ...
            preflight_response = security_test_client.client.options(
            ...
            if preflight_response.status_code == 200:
                ...
                assert allowed_origin != origin or origin in ["http://localhost:3000"], (
                    f"Should not allow malicious origin: {origin}"
                )
    ...
    def test_content_type_manipulation_bypass_prevention(
        ...
        for content_type in content_type_attacks:
            ...
            response = security_test_client.client.post(
                "/api/v1/clients", data=malicious_data, headers=headers
            )

            # Should properly validate content type and reject malformed requests
            assert response.status_code in [400, 415, 422], (
                f"Should reject inappropriate content type: {content_type}"
            )
    ...
    def test_user_agent_spoofing_bypass_prevention(
        ...
        for user_agent in spoofed_agents:
            ...
            assert response.status_code == 403, (
                f"User agent spoofing should not bypass authorization: {user_agent}"
            )
    ...
    def test_encoding_manipulation_bypass_prevention(
        ...
        for encoded_data in encoding_attacks:
            # Add required fields
            full_data = {**encoded_data, "cpf": "123456789", "birth_date": "1990-01-01"}

            response = security_test_client.client.post(
                "/api/v1/clients", json=full_data, headers=headers
            )
            ...
            if response.status_code == 201:
                ...
                assert "role" not in created_client or created_client["role"] != "admin", (
                    f"Encoding manipulation should not grant admin role: {encoded_data}"
                )
