...
class TestSQLInjectionPrevention:
    ...
    def test_client_creation_sql_injection_prevention(
        ...
        response = security_test_client.client.post(
            "/api/v1/clients", json=malicious_client_data, headers=authenticated_admin_headers
        )

        # Should reject malicious input with proper error (400 from middleware, 422 from validation)
        assert response.status_code in [400, 422], (
            f"Expected security error (400) or validation error (422), got {response.status_code}"
        )

        # Verify no SQL injection occurred by checking database state
        from sqlalchemy import text

        clients_count = test_session.execute(text("SELECT COUNT(*) FROM agent1_clients")).scalar()
    ...
    def test_user_creation_sql_injection_prevention(
        ...
        response = security_test_client.client.post(
            "/api/v1/users", json=malicious_user_data, headers=authenticated_sysadmin_headers
        )
    ...
    @pytest.mark.parametrize("sql_payload", ["' OR '1'='1", "'; DROP TABLE users; --", "admin'--"])
    def test_login_sql_injection_prevention(
        self, sql_payload: str, security_test_client, test_regular_user: User, test_session: Session
    ):
        """Test that login endpoint properly rejects SQL injection attempts."""
        # Attempt SQL injection in login credentials
        response = security_test_client.client.post(
            "/api/v1/auth/login", json={"email": sql_payload, "password": sql_payload}
        )

        # Should reject with authentication error (not crash)
        assert response.status_code in [400, 401, 422], (
            f"Expected security, auth, or validation error, got {response.status_code}"
        )
        ...
        if response.status_code == 200:
            ...
            assert "access_token" not in response_data, (
                "Should not return access token for malicious login"
            )
...
class TestXSSPrevention:
    ...
    def test_client_data_xss_prevention(
        ...
        if response.status_code == 201:
            ...
            assert "onerror=" not in client_data.get("name", ""), (
...
class TestPathTraversalPrevention:
    ...
    def test_file_access_path_traversal_prevention(
        ...
        response = security_test_client.client.get(
            f"/api/v1/files?path={path_payload}", headers=authenticated_admin_headers
        )

        # Should reject path traversal attempts
        assert response.status_code in [400, 403, 404], (
            f"Expected error for path traversal, got {response.status_code}"
        )
...
class TestCommandInjectionPrevention:
    ...
    def test_system_command_injection_prevention(
        ...
        response = security_test_client.client.post(
            "/api/v1/clients", json=malicious_data, headers=authenticated_admin_headers
        )
...
class TestJSONInjectionPrevention:
    ...
    def test_json_structure_manipulation_prevention(
        self, security_test_client, authenticated_admin_headers: dict[str, str]
    ):
        ...
        response = security_test_client.client.post(
            "/api/v1/clients", json=malicious_json, headers=authenticated_admin_headers
        )
...
class TestHeaderInjectionPrevention:
    ...
    def test_header_injection_prevention(
        self, header_payload: str, security_test_client, test_regular_user: User
    ):
        ...
        response = security_test_client.client.get("/api/v1/auth/me", headers=malicious_headers)
        ...
        if response.status_code == 200:
            ...
            assert (
                "Set-Cookie" not in str(response_headers).lower()
                or "admin=true" not in str(response_headers).lower()
            ), "Malicious cookie should not be set"
...
class TestMassAssignmentPrevention:
    ...
    async def test_user_mass_assignment_prevention(
        self, security_test_client, test_regular_user: User, user_auth_headers: dict[str, str]
    ):
        ...
        if response.status_code == 200:
            ...
            assert updated_user.get("role") != "sysadmin", (
                "Role should not be updatable via mass assignment"
            )
            assert updated_user.get("user_id") == str(test_regular_user.user_id), (
                "User ID should not be changeable"
            )
            ...
            assert updated_user.get("full_name") == "Updated Name", (
                "Legitimate fields should be updatable"
            )
        else:
            # Should at minimum not crash the application
            assert response.status_code in [400, 403, 422], (
                f"Expected controlled security error, got {response.status_code}"
            )
