...
class TestDirectRoleEscalationPrevention:
    ...
    def test_user_cannot_escalate_to_admin_via_profile_update(
        ...
        response = security_test_client.client.put(
            f"/api/v1/users/{regular_user.user_id}", json=escalation_data, headers=headers
        )
        ...
        assert response.status_code in [403, 422, 404], (
            f"Role escalation should be rejected, got {response.status_code}"
        )
    ...
    def test_admin_cannot_escalate_to_sysadmin_via_profile_update(
        ...
        response = security_test_client.client.put(
            f"/api/v1/users/{admin_user.user_id}", json=escalation_data, headers=headers
        )

        # Should be rejected (403=forbidden, 422=validation error, 404=not found - all indicate failure)
        assert response.status_code in [403, 422, 404], (
            f"Sysadmin escalation should be rejected, got {response.status_code}"
        )
    ...
    def test_user_cannot_escalate_other_users_roles(
        ...
        response = security_test_client.client.put(
            f"/api/v1/users/{target_user.user_id}", json=escalation_data, headers=headers
        )

        # Should be rejected due to insufficient permissions (403=forbidden, 422=validation error, 404=not found - all indicate failure)
        assert response.status_code in [403, 422, 404], (
            f"Cross-user role escalation should be forbidden, got {response.status_code}"
        )
...
class TestJWTTokenRoleManipulation:
    ...
    @pytest.mark.parametrize(
        "manipulated_role", ["sysadmin", "admin", "super_user", "root", "administrator"]
    )
    def test_manipulated_jwt_role_rejection(
        ...
        for malicious_token in malicious_jwt_tokens:
            ...
            assert response.status_code == 401, (
                f"Manipulated JWT should be rejected, got {response.status_code}"
            )

            # Should not return user data
            if response.status_code == 200:
                user_data = response.json()
                assert user_data.get("role") != manipulated_role, (
                    "Manipulated role should not be accepted"
                )
    ...
    def test_token_signature_manipulation_detection(
        ...
        if len(token_parts) == 3:
            ...
            manipulated_signature = token_parts[2][:-1] + (
                "X" if token_parts[2][-1] != "X" else "Y"
            )
    ...
    def test_algorithm_none_attack_prevention(
        ...
        assert response.status_code == 401, (
            f"Algorithm none attack should be rejected, got {response.status_code}"
        )
...
class TestAgentPermissionBoundaryViolation:
    ...
    def test_user_cannot_bypass_client_management_permissions(
        ...
        for method, endpoint, data in endpoints_to_test:
            ...
            assert response.status_code == 403, (
                f"Access to {endpoint} should be forbidden, got {response.status_code}"
            )
    ...
    def test_user_cannot_escalate_agent_permissions_directly(
        ...
        response = security_test_client.client.post(
            "/api/v1/permissions/assign", json=permission_data, headers=headers
        )

        # Should be forbidden - regular users cannot assign permissions
        assert response.status_code == 403, (
            f"Self permission assignment should be forbidden, got {response.status_code}"
        )
        ...
        assert not permission_check, (
            "No permissions should have been granted through self-assignment"
        )
    ...
    def test_admin_cannot_grant_sysadmin_level_permissions(
        ...
        response = security_test_client.client.post(
            "/api/v1/permissions/assign-system", json=system_permission_data, headers=headers
        )

        # Should be forbidden - only sysadmin can grant system permissions
        assert response.status_code in [403, 404], (
            f"System permission assignment should be forbidden for admin, got {response.status_code}"
        )
...
class TestCrossUserPermissionExploitation:
    ...
    def test_user_cannot_access_other_user_data_via_permission_bypass(
        ...
        response = security_test_client.client.get(
            f"/api/v1/users/{user2.user_id}", headers=headers
        )

        # Should be forbidden - users can only access their own data (403=forbidden, 404=not found - both indicate access denied)
        assert response.status_code in [403, 404], (
            f"Cross-user data access should be forbidden, got {response.status_code}"
        )
    ...
    def test_permission_inheritance_exploitation_prevention(
        ...
        response = security_test_client.client.post(
            "/api/v1/clients", json=impersonation_data, headers=headers
        )

        # Should reject impersonation attempt
        assert response.status_code in [400, 403, 422], (
            f"Impersonation should be rejected, got {response.status_code}"
        )
...
class TestAdministrativeFunctionAbuse:
    ...
    def test_bulk_permission_assignment_abuse_prevention(
        ...
        bulk_assignment_data = {
            ...
            "permissions": {
                ...
                "all_agents": {"create": True, "read": True, "update": True, "delete": True},
            },
        }

        response = security_test_client.client.post(
            "/api/v1/permissions/bulk-assign", json=bulk_assignment_data, headers=headers
        )
        ...
        if response.status_code == 200:
            ...
            assert response.status_code in [400, 403, 422], (
                f"Bulk privilege escalation should be rejected, got {response.status_code}"
            )
    ...
    def test_permission_template_abuse_prevention(
        ...
        response = security_test_client.client.post(
            "/api/v1/permissions/templates", json=malicious_template, headers=headers
        )
        ...
        if response.status_code == 201:
            ...
            assert "role" not in permissions, (
                "Role field should not be allowed in permission templates"
            )
            assert "system:config" not in permissions, (
                "System permissions should not be allowed for admin users"
            )
            assert "system:logs" not in permissions, (
                "System permissions should not be allowed for admin users"
            )
        else:
            # Should be rejected if admin lacks template creation permissions
            assert response.status_code in [403, 422], (
                f"Malicious template should be rejected, got {response.status_code}"
            )
...
class TestPermissionStateManipulation:
    ...
    async def test_concurrent_permission_manipulation_prevention(
        ...
        async def grant_permission():
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    "http://localhost:8000/api/v1/permissions/assign", json=data, headers=headers
                ) as resp:
                    return resp.status

        async def revoke_permission():
            async with aiohttp.ClientSession() as session:
                data = {"user_id": str(target_user.user_id), "agent_name": "client_management"}
                async with session.post(
                    "http://localhost:8000/api/v1/permissions/revoke", json=data, headers=headers
                ) as resp:
                    return resp.status
        ...
        try:
            ...
            if final_permissions:
                assert isinstance(final_permissions.permissions, dict), (
                    "Permission state should not be corrupted"
                )
                # Verify it has valid structure
                assert (
                    "create" in final_permissions.permissions
                    or "read" in final_permissions.permissions
                ), "Permission data should be valid"
    ...
    def test_session_state_confusion_prevention(
        ...
        if response.status_code == 200:
            ...
            assert user_data["user_id"] == str(regular_user.user_id), (
                "Should return token user, not session user"
            )
            assert user_data["role"] == regular_user.role.value, (
                "Should return token role, not session role"
            )
            assert user_data["email"] == regular_user.email, (
                "Should return token email, not session email"
            )
        else:
            # Session confusion should be detected and rejected
            assert response.status_code == 401, (
                f"Confused session should be rejected, got {response.status_code}"
            )
