...
class TestSensitiveDataExposurePrevention:
    ...
    def test_cpf_masking_in_client_responses(
        ...
        from src.tests.factories import generate_valid_cpf

        test_cpf = generate_valid_cpf()
        test_client = ClientFactory(
            full_name="Test Client",
            cpf=test_cpf,  # Use valid CPF generated by factory
            birth_date=date(1990, 1, 1),  # Use proper date object
        )
        ...
        response = security_test_client.client.get(
            f"/api/v1/clients/{test_client.client_id}", headers=headers
        )
        ...
        if response.status_code == 200:
            ...
            assert test_cpf not in cpf_value, "Full CPF should not be exposed"
            # Remove formatting to check unformatted version too
            unformatted_cpf = test_cpf.replace(".", "").replace("-", "")
            assert unformatted_cpf not in cpf_value, "Unformatted CPF should not be exposed"
            ...
            is_masked = (
                ...
                or "*" in cpf_value  # ***.***.***-89
                or "###" in cpf_value  # ###.###.###-89
                or cpf_value.startswith("***")  # ***456789
                or len(
                    cpf_value.replace("-", "").replace("X", "").replace("*", "").replace("#", "")
                )
                < 9  # Some digits hidden
            )
    ...
    def test_password_data_never_exposed(
        ...
        if users_response.status_code == 200:
            ...
            if isinstance(users_data, list):
                users_to_check = users_data
            elif isinstance(users_data, dict):
                # Could be a single user response, or paginated response with 'items'/'data'
                if "items" in users_data:
                    users_to_check = users_data["items"]
                elif "data" in users_data:
                    users_to_check = users_data["data"]
                else:
                    # Single user response wrapped in dict
                    users_to_check = [users_data]
            else:
                raise AssertionError(f"Unexpected response format: {type(users_data)}")
            ...
            for user_data in users_to_check:
                ...
                if not isinstance(user_data, dict):
                    raise AssertionError(
                        f"Expected user data to be dict, got: {type(user_data)} - {user_data}"
                    )
                ...
                for field in sensitive_fields:
                    assert field not in user_data, (
                        f"Sensitive field '{field}' should not be in user response"
                    )
                ...
                for key, value in user_data.items():
                    if isinstance(value, str):
                        ...
                        assert not re.match(r"^\$2[abyxz]?\$\d+\$", value), (
                            f"Field '{key}' contains what looks like a password hash"
                        )
                        # Check for other hash patterns
                        assert not (len(value) == 60 and value.startswith("$2")), (
                            f"Field '{key}' may contain a bcrypt hash"
                        )
    ...
    def test_authentication_tokens_not_logged_or_exposed(
        ...
        error_response = security_test_client.client.get(
            "/api/v1/nonexistent-endpoint", headers=headers
        )
        ...
        for part in token_parts:
            assert part not in response_text, (
                f"Token part should not be exposed in error response: {part[:10]}..."
            )
        ...
        if error_response.status_code >= 400:
            try:
                ...
                assert "authorization" not in response_text, (
                    "Error should not expose authorization details"
                )
    ...
    def test_internal_ids_and_system_info_not_exposed(
        ...
        endpoints_to_test = ["/api/v1/users", "/api/v1/clients", "/api/v1/auth/me"]
        ...
        for endpoint in endpoints_to_test:
            ...
            if response.status_code == 200:
                ...
                sensitive_system_info = [
                    ...
                    "session_id",
                    "jti",  # Token-related internal IDs
                ]
                ...
                for sensitive_info in sensitive_system_info:
                    if sensitive_info in data_str:
                        ...
                        if admin_user.role == UserRole.SYSADMIN and sensitive_info in [
                            "created_by_user_id"
                        ]:
                            continue  # Acceptable for sysadmin
                        else:
                            raise AssertionError(
                                f"Sensitive system info '{sensitive_info}' found in {endpoint} response"
                            )
...
class TestErrorMessageInformationDisclosure:
    ...
    def test_database_error_information_not_disclosed(
        ...
        database_error_triggers = [
            ...
            # Invalid foreign key attempt
            {
                ...
                "created_by_user_id": "invalid-uuid",
            },
            # Invalid data type
        ...
        for client_data in database_error_triggers:
            response = security_test_client.client.post(
                "/api/v1/clients", json=client_data, headers=headers
            )
            ...
            if response.status_code >= 400:
                try:
                    ...
                    for internal_term in database_internals:
                        assert internal_term not in error_detail, (
                            f"Database internal '{internal_term}' exposed in error: {error_detail}"
                        )
                    ...
                    assert "postgresql" not in response_text, (
                        "Database type should not be exposed in errors"
                    )
            ...
            if response.status_code >= 400:
                ...
                for internal_path in filesystem_internals:
                    if internal_path in response_text:
                        ...
                        if (
                            internal_path in ["file not found", "permission denied"]
                            and "path" not in response_text
                        ):
                            continue
                        raise AssertionError(
                            f"File system internal '{internal_path}' exposed in error response"
                        )
    ...
    def test_validation_error_information_disclosure_prevention(
        ...
        for invalid_data in validation_triggers:
            response = security_test_client.client.post(
                "/api/v1/clients", json=invalid_data, headers=headers
            )
                    ...
                    if "detail" in error_data:
                        ...
                        for internal in internal_validators:
                            assert internal not in error_detail, (
                                f"Internal validator '{internal}' exposed in validation error"
                            )
...
class TestUserEnumerationPrevention:
    ...
    def test_login_response_consistency_for_user_enumeration_prevention(
        ...
        existing_user_response = security_test_client.client.post(
            "/api/v1/auth/login", json={"email": existing_user.email, "password": "wrong_password"}
        )

        # Test login with non-existent user
        non_existent_response = security_test_client.client.post(
            "/api/v1/auth/login", json={"email": non_existent_email, "password": "wrong_password"}
        )
        ...
        if existing_user_response.status_code == 401:
            ...
            assert "user not found" not in existing_error.lower(), (
                "Should not explicitly mention user not found"
            )
            assert "invalid user" not in existing_error.lower(), "Should not mention invalid user"
            assert "user does not exist" not in non_existing_error.lower(), (
                "Should not mention user existence"
            )

            # Messages should be similar (ideally identical)
            assert len(existing_error) > 0 and len(non_existing_error) > 0, (
    ...
    def test_password_reset_user_enumeration_prevention(
        ...
        existing_response = security_test_client.client.post(
            "/api/v1/auth/forgot-password", json={"email": existing_user.email}
        )

        # Test password reset for non-existent user
        non_existing_response = security_test_client.client.post(
            "/api/v1/auth/forgot-password", json={"email": non_existent_email}
        )
        ...
        if existing_response.status_code == 200:
            ...
            assert "user not found" not in existing_message.lower(), (
                "Should not mention user not found"
            )
            assert "email not found" not in non_existing_message.lower(), (
                "Should not mention email not found"
            )

            # Should provide generic success message for both
            assert "sent" in existing_message.lower() or "email" in existing_message.lower(), (
                "Should suggest email was sent"
            )
            assert (
                "sent" in non_existing_message.lower() or "email" in non_existing_message.lower()
            ), "Should suggest email was sent"
    ...
    def test_user_profile_access_enumeration_prevention(
        ...
        invalid_uuid_responses = [
            (uid, resp) for uid, resp in responses if not self._is_valid_uuid(uid)
        ]
        ...
        if invalid_uuid_responses:
            ...
            assert len(set(status_codes)) <= 1, (
                "Invalid UUIDs should return consistent status codes"
            )
        ...
        if valid_uuid_responses:
            ...
            assert all(code in [403, 404] for code in status_codes), (
                "Non-existent valid UUIDs should return 403 or 404"
            )
...
class TestCrossUserDataBleeding:
    ...
    def test_client_data_isolation_between_users(
        ...
        client1 = ClientFactory(
            name="User1 Confidential Client", cpf="111111111", created_by_user_id=user1.user_id
        )

        client2 = ClientFactory(
            name="User2 Secret Client", cpf="222222222", created_by_user_id=user2.user_id
        )
        ...
        if response1.status_code == 200:
            ...
            assert "User2 Secret Client" not in client_names, (
                "Should not see other user's client names"
            )
            assert "222222222" not in str(client_cpfs), "Should not see other user's client CPFs"
            assert "***.***.***-22" not in str(client_cpfs), (
                "Should not see other user's masked CPFs"
            )
    ...
    def test_session_data_isolation_between_concurrent_users(
        ...
        token2 = auth_service.create_access_token(
            user_id=user2.user_id, user_role=user2.role.value, user_email=user2.email
        )
    ...
    def test_permission_state_isolation_between_users(
        ...
        assert create_response1_again.status_code == 403, (
            "User1 permissions should not have escalated after user2's actions"
        )
            ...
            if response.status_code >= 400:
                ...
                for debug_info in debug_indicators:
                    if debug_info in response_text:
                        ...
                        raise AssertionError(
                            f"Debug information '{debug_info}' exposed in error response from {endpoint}"
                        )
