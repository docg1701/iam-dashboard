...
def attack_user_scenarios(test_session: Session) -> dict[str, User]:
    ...
    users["regular_user"] = UserFactory(
        role=UserRole.USER, email="attacker@test.com", is_active=True
    )

    # Admin user (escalation target)
    users["admin_user"] = UserFactory(role=UserRole.ADMIN, email="admin@test.com", is_active=True)

    # Sysadmin user (ultimate target)
    users["sysadmin_user"] = UserFactory(
        role=UserRole.SYSADMIN, email="sysadmin@test.com", is_active=True
    )

    # Disabled user (should never get access)
    users["disabled_user"] = UserFactory(
        role=UserRole.USER, email="disabled@test.com", is_active=False
    )

    # Locked user (brute force victim)
    users["locked_user"] = UserFactory(role=UserRole.USER, email="locked@test.com", is_active=True)
...
def security_test_client(client):
    ...
    class SecurityTestClient:
        ...
        def attempt_privilege_escalation(self, token: str, target_role: str):
            ...
            return self.client.post("/api/v1/users/me", json={"role": target_role}, headers=headers)
