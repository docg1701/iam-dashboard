...
class TestBruteForceProtection:
    ...
    def test_account_lockout_after_failed_attempts(
        ...
        for i, response in enumerate(responses[:-1]):
            assert response.status_code == 401, f"Failed attempt {i+1} should return 401, got {response.status_code}"
        
        # Final attempt should trigger lockout
        final_response = responses[-1]
        assert final_response.status_code in [423, 429], f"Account lockout should be triggered, got {final_response.status_code}"
        
        # Verify account is locked even with correct password
        correct_password_response = security_test_client.client.post("/api/v1/auth/login", json={
            "email": target_user.email,
            "password": "correct_password"  # This would be the actual password
        })
        
        assert correct_password_response.status_code in [423, 429], "Locked account should reject even correct password"
        ...
        for ip in ip_addresses:
            ...
            for attempt in range(3):
                response = security_test_client.client.post(
                    ...
                    json={
                        "email": target_user.email,
                        "password": f"attack_from_{ip}_{attempt}"
                    },
                    headers=headers
                )
    ...
    def test_brute_force_with_redis_failure_graceful_degradation(
        ...
        for attempt in range(8):
            response = security_test_client.client.post("/api/v1/auth/login", json={
                "email": target_user.email,
                "password": f"failed_attempt_{attempt}"
            })
            
            # Should still protect against brute force using database tracking
            if attempt >= 5:  # After reasonable threshold
                assert response.status_code in [401, 423, 429], f"Should still protect after attempt {attempt}"
...
class TestJWTSecurityAttacks:
    ...
    def test_jwt_signature_tampering_detection(
        ...
        for tampered_token in signature_attacks:
            ...
            assert response.status_code == 401, f"Tampered token should be rejected: {tampered_token[:50]}..."
    ...
    def test_jwt_algorithm_confusion_attack_prevention(
        ...
        for malicious_header in algorithm_attacks:
            ...
            assert response.status_code == 401, f"Algorithm confusion should be rejected: {malicious_header}"
...
class TestTwoFactorAuthenticationBypass:
    ...
    def test_2fa_code_brute_force_protection(
        ...
        for code in possible_codes[:20]:  # Limit to reasonable number for test
            response = security_test_client.client.post("/api/v1/auth/verify-2fa", json={
    ...
    def test_2fa_session_fixation_prevention(
        ...
        for session_id in predicted_sessions:
            ...
            assert response.status_code in [400, 401, 404], f"Invalid session should be rejected: {session_id}"
...
class TestSessionSecurityAttacks:
    ...
    def test_concurrent_session_abuse_prevention(
        ...
        if user_data_list:
            ...
            user_ids = set(data["user_id"] for data in user_data_list)
            assert len(user_ids) == 1, "Concurrent sessions should not allow impersonation"
            assert str(target_user.user_id) in user_ids, "All sessions should be for the correct user"
    ...
    def test_session_timeout_enforcement(
        ...
        if immediate_response.status_code == 200:
            ...
            assert delayed_response.status_code in [200, 401], "Session timeout should be handled properly"
...
class TestTimingAttackResistance:
    ...
    def test_login_timing_consistency(
        ...
        for _ in range(5):  # Multiple samples for timing analysis
            ...
            start_time = time.time()
            response1 = security_test_client.client.post("/api/v1/auth/login", json={
            ...
            start_time = time.time()
            response2 = security_test_client.client.post("/api/v1/auth/login", json={
    ...
    def test_password_verification_timing_consistency(
        ...
        assert timing_variance < 0.1, f"Password verification timing variance too large: {timing_variance}s"