...
- @patch('src.core.permissions.PermissionService') - Internal service mocking 
- Mock return values for security logic
- Fake permission behavior testing - SECURITY CRITICAL VIOLATION
...
def create_test_user_with_permissions(session: Session, role: UserRole, agent_permissions: dict) -> User:
...
class TestPermissionDecoratorsRefactored:
    ...
    async def test_decorator_with_user_id_kwarg_real_security(self, mock_redis, test_session: Session) -> None:
        """
        ...
        # Patch PermissionService to use test session
        with patch('src.core.permissions.PermissionService') as MockPermissionService:
            # Create a real PermissionService instance with test session
            real_service = PermissionService(session=test_session)
            MockPermissionService.return_value = real_service
    ...
    @pytest.mark.asyncio
    @patch('src.core.security.redis')  # ✅ External boundary only  
    async def test_decorator_with_user_object_kwarg_real_security(self, mock_redis, test_session: Session) -> None:
        """
    ...
    @pytest.mark.asyncio
    @patch('src.core.security.redis')  # ✅ External boundary only
    async def test_decorator_unauthorized_access_real_security_rejection(self, mock_redis, test_session: Session) -> None:
        """
        ...
        @permission_required(AgentName.PDF_PROCESSING, "create")  # PDF processing not granted to regular users
        async def test_function(user_id: UUID, data: str) -> str:
            return f"Success: {data}"

        # Execute and expect REAL security rejection
        with pytest.raises(AuthorizationError, match="does not have create permission for pdf_processing"):
            await test_function(user_id=user.user_id, data="test")

    @pytest.mark.asyncio
    @patch('src.core.security.redis')  # ✅ External boundary only
    async def test_decorator_sysadmin_bypass_real_logic(self, mock_redis, test_session: Session) -> None:
        """
...
class TestRolePemissionCheckingFunctionsRefactored:
    ...
    @pytest.mark.asyncio
    @patch('src.core.security.redis')  # ✅ External boundary only
    async def test_check_admin_or_sysadmin_with_admin_real_logic(self, mock_redis, test_session: Session) -> None:
        """
    ...
    @pytest.mark.asyncio
    @patch('src.core.security.redis')  # ✅ External boundary only
    async def test_check_admin_or_sysadmin_with_user_real_rejection(self, mock_redis, test_session: Session) -> None:
        """
    ...
    @pytest.mark.asyncio
    @patch('src.core.security.redis')  # ✅ External boundary only
    async def test_check_sysadmin_only_with_sysadmin_real_logic(self, mock_redis, test_session: Session) -> None:
        """
    ...
    @pytest.mark.asyncio
    @patch('src.core.security.redis')  # ✅ External boundary only
    async def test_check_sysadmin_only_with_admin_real_rejection(self, mock_redis, test_session: Session) -> None:
        """
