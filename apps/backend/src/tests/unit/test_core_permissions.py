...
class TestPermissionMiddleware:
    ...
    async def test_call_success(self, test_session: Session) -> None:
...
class TestCheckUserPermissionSync:
    ...
    def test_sync_permission_check_no_loop(self, test_session: Session) -> None:
    ...
    def test_sync_permission_check_with_stopped_loop(self, test_session: Session) -> None:
        ...
        test_user = User(
            user_id=user_id,
            email="stopped_loop_test@example.com",
            role=UserRole.USER,
            is_active=True,
            password_hash="test_hash",
            full_name="Stopped Loop Test User",
        )
        test_session.add(test_user)
        test_session.commit()
        ...
        # Mock the loop to return False (no permissions)
        async def mock_permission_check():
            service = PermissionService(session=test_session)
            return await service.check_user_permission(user_id, AgentName.CLIENT_MANAGEMENT, "read")
...
class TestPermissionRequiredDecorator:
    ...
    async def test_decorator_with_user_id_kwarg(self, test_session: Session) -> None:
        ...
        test_user = User(
            ...
            role=UserRole.ADMIN,  # Give admin role for permissions
            is_active=True,
            password_hash="test_hash",
    ...
    async def test_decorator_with_user_id_positional_arg(self, test_session: Session) -> None:
        ...
        test_user = User(
            user_id=user_id,
            email="positional_test@example.com",
            role=UserRole.ADMIN,  # Give admin role for permissions
            is_active=True,
            password_hash="test_hash",
            full_name="Positional Test User",
        )
        test_session.add(test_user)
        test_session.commit()
    ...
    async def test_decorator_with_user_object_positional_arg(self, test_session: Session) -> None:
        ...
        user = User(
            user_id=uuid4(),
            email="pos_object_test@example.com",
            password_hash="hash",
            role=UserRole.ADMIN,  # Give admin role for permissions
            is_active=True,
            full_name="Positional Object Test User"
        )
        test_session.add(user)
        test_session.commit()
    ...
    async def test_decorator_permission_denied(self, test_session: Session) -> None:
        """Test decorator raises AuthorizationError when permission denied."""
        user_id = uuid4()

        # Create test user without sufficient permissions
        test_user = User(
            user_id=user_id,
            email="denied_test@example.com",
            role=UserRole.USER,  # Regular user without create permissions
            is_active=True,
            password_hash="test_hash",
