...
- Internal service patching patterns eliminated - Real service operations used
- Mock return values for business logic replaced with real operations
- Fake service behavior testing replaced with real API testing
...
class TestListUsersRefactored:
    ...
    async def test_list_users_success_with_pagination_real_logic(
        self, mock_audit, mock_redis
    ) -> None:
        """
        ...
        mock_token_data = TokenData(
            user_id=uuid4(), role="sysadmin", email="sysadmin@test.com"
        )  # Use sysadmin to bypass permission checks
...
class TestCreateUserRefactored:
    ...
    @patch("src.utils.audit.log_database_action")  # ✅ External audit boundary
    async def test_create_user_success_real_business_logic(
        self, mock_audit, mock_hash, mock_redis
    ) -> None:
        """
    ...
    @patch("src.core.security.auth_service.get_password_hash")  # ✅ External crypto boundary
    @patch("src.utils.audit.log_database_action")  # ✅ External audit boundary
    async def test_create_user_duplicate_email_real_error_handling(
        self, mock_audit, mock_hash, mock_redis
    ) -> None:
        """
